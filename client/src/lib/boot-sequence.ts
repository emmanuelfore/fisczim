import { getDb, isStorageBroken } from "@/lib/offline-db";

// Local health probes (main's offline-db predates checkStorageHealth/ensurePersistentStorage).
async function checkStorageHealth(): Promise<boolean> {
  try {
    await getDb();
    return !isStorageBroken();
  } catch {
    return false;
  }
}
