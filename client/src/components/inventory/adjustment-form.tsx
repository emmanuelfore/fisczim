...
const adjustmentSchema = z.object({
  ...
  actualQuantity: z
    .string()
    .min(1, "Actual quantity required")
    .refine(
...
export function AdjustmentForm() {
  ...
  const systemQuantity = Number(
    (selectedProduct as any)?.branchStock ?? selectedProduct?.stockLevel ?? 0,
  );
  const quantityChange =
    String(actualQuantityValue ?? "").trim() && Number.isFinite(actualQuantity)
      ? actualQuantity - systemQuantity
      : null;

  const onSubmit = (values: AdjustmentFormValues) => {
    const selected = products?.find(
      (product) => product.id.toString() === values.productId,
    );
    const currentStock = Number(
      (selected as any)?.branchStock ?? selected?.stockLevel ?? 0,
    );
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      ...
                      className="h-12 font-mono text-lg border-slate-200 rounded-xl focus:ring-orange-500/20"
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Textarea
                      ...
                      className="min-h-[80px] resize-none border-slate-200 rounded-xl focus:ring-orange-500/20"
                    />
                  </FormControl>
                  <FormMessage />
                </FormItem>
