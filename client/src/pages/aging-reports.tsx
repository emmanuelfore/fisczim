...
export default function AgingReportsPage() {
  ...
  const getSelectedTab = () => {
    ...
    return location.includes("accounts-payable") || search.includes("tab=ap")
      ? "ap"
      : "ar";
  };
    ...
    <Layout hideHeaderTitle>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className=" font-semibold text-slate-600">As Of Date:</div>
            <Input
              type="date"
              value={asOfDate}
        ...
        >
          ...
          <TabsContent
            value="ar"
            className="m-0 space-y-6 outline-none focus:ring-0"
          >
            <div className="grid grid-cols-1 gap-6">
              <Card className="rounded-2xl border-slate-200">
                <CardHeader>
                  ...
                  <CardDescription>
                    Customers owing money to the company
                  </CardDescription>
                </CardHeader>
                        ...
                        <>
                          {arData?.map((row) => (
                            <TableRow
                              ...
                              onClick={() =>
                                setLocation(
                                  `/accounting/debtors/${row.customerId}`,
                                )
                              }
                            >
                          ...
                          {arTotals && (
                            <TableRow className="bg-slate-50/80 hover:bg-slate-50/80 border-t border-slate-200">
                              <TableCell className="pl-6 font-black  uppercase text-slate-800">
                                Total Receivables
                              </TableCell>
                              <TableCell className="text-right font-bold text-slate-800">
                                {formatCurrency(arTotals.current)}
                              </TableCell>
                              <TableCell className="text-right font-bold text-amber-600">
                                {formatCurrency(arTotals.days30)}
                              </TableCell>
                              <TableCell className="text-right font-bold text-orange-500">
                                {formatCurrency(arTotals.days60)}
                              </TableCell>
                              <TableCell className="text-right font-bold text-orange-600">
                                {formatCurrency(arTotals.days90)}
                              </TableCell>
                              <TableCell className="text-right font-black text-rose-600">
                                {formatCurrency(arTotals.over90)}
                              </TableCell>
                              <TableCell className="text-right pr-6 font-black text-xl text-emerald-600">
                                {formatCurrency(arTotals.total)}
                              </TableCell>
                            </TableRow>
                          )}
                        </>
                        ...
                        <>
                          {apData?.map((row) => (
                            <TableRow
                              ...
                              onClick={() =>
                                setLocation(
                                  `/accounting/creditors/${row.supplierId}`,
                                )
                              }
                            >
                              <TableCell className="pl-6 font-bold text-slate-800">
                          ...
                          {apTotals && (
                            <TableRow className="bg-slate-50/80 hover:bg-slate-50/80 border-t border-slate-200">
                              ...
                              <TableCell className="text-right font-bold text-slate-800">
                                {formatCurrency(apTotals.current)}
                              </TableCell>
                              <TableCell className="text-right font-bold text-amber-600">
                                {formatCurrency(apTotals.days30)}
                              </TableCell>
                              <TableCell className="text-right font-bold text-orange-500">
                                {formatCurrency(apTotals.days60)}
                              </TableCell>
                              <TableCell className="text-right font-bold text-orange-600">
                                {formatCurrency(apTotals.days90)}
                              </TableCell>
                              <TableCell className="text-right font-black text-rose-600">
                                {formatCurrency(apTotals.over90)}
                              </TableCell>
                              <TableCell className="text-right pr-6 font-black text-xl text-rose-600">
                                {formatCurrency(apTotals.total)}
                              </TableCell>
                            </TableRow>
                          )}
                        </>
