...
const priceAdjustmentSchema = z.object({
    productId: z.number(),
    newPrice: z.string().transform((v) => parseFloat(v)).pipe(z.number().positive()),
...
export function PriceAdjustmentDialog({ product, companyId }: { product: Product, companyId: number }) {
    ...
    const { data: history, isLoading: historyLoading } = usePriceHistory(product.id);
                ...
                <div className="flex flex-col md:flex-row h-full relative">
                    ...
                    <div className="w-[280px] bg-slate-50 border-l border-slate-100 flex flex-col">
                        <div className="p-6 border-b border-slate-200 bg-white/50 backdrop-blur-sm sticky top-0 z-10">
                            ...
                            <p className="text-[11px] text-slate-500 font-medium">Historical price changes</p>
                        </div>
                            ...
                            <div className="p-4 space-y-4">
                                {historyLoading ? (
                                    ...
                                    <div className="text-center p-8">
                                        ...
                                        <p className="text-xs font-semibold text-slate-400">No previous changes</p>
                                    </div>
                                ) : (
                                        ...
                                        <div key={item.id} className="relative pl-6">
                                            ...
                                            <div className="space-y-1 mb-6">
                                                <div className="flex items-baseline justify-between gap-2">
                                                    <span className="text-sm font-black text-slate-800">${parseFloat(item.newPrice).toFixed(2)}</span>
                                                    <span className="text-[10px] font-bold text-slate-400">{format(new Date(item.createdAt), 'MMM d, yyyy')}</span>
                                                </div>
                                                <div className="flex items-center gap-1 text-[10px] font-bold text-slate-400 mb-1">
                                                    <span className="line-through">${parseFloat(item.oldPrice).toFixed(2)}</span>
                                                    <span>→</span>
