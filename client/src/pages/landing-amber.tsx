...
function FZLogo({ size = 36, mono = false }: { size?: number; mono?: boolean }) {
    ...
    const g1e = mono ? "#cccccc" : "#FFD740";
    const g2s = mono ? "#dddddd" : "#0A3FC0";
    const g2e = mono ? "#ffffff" : "#FFB74D";
...
export default function FiscalZoneLanding() {
    return (
        ...
        :root {
          --blue: #E07800; --blue-bright: #FF9500; --blue-light: #FFB74D;
          --cyan: #FFD740; --cyan-dim: rgba(255,215,64,0.1);
          --blue-pale: rgba(224,120,0,0.12); --blue-dim: rgba(224,120,0,0.08);
          --green: #00CC66;
          --bg: #0a0600; --surface: rgba(255,255,255,0.03);
          --border: rgba(224,120,0,0.2); --border-b: rgba(255,183,77,0.14);
          --text: #fff5e0; --muted: rgba(255,245,224,0.48);
        }
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        /* BG */
        .bg { position:fixed; inset:0; z-index:0; pointer-events:none; overflow:hidden; background:#0a0600; }
        .bg-orb { position:absolute; border-radius:50%; filter:blur(90px); will-change:transform; }
        .bg-o1 { width:800px; height:800px; background:radial-gradient(circle, rgba(224,120,0,0.18) 0%, transparent 70%); top:-220px; right:-200px; animation:d1 24s ease-in-out infinite; }
        .bg-o2 { width:600px; height:600px; background:radial-gradient(circle, rgba(255,149,0,0.1) 0%, transparent 70%); bottom:-160px; left:-140px; animation:d2 30s ease-in-out infinite; }
        .bg-o3 { width:400px; height:400px; background:radial-gradient(circle, rgba(224,120,0,0.07) 0%, transparent 70%); top:48%; left:36%; filter:blur(70px); animation:d3 19s ease-in-out infinite; }
        ...
        .bg-grid { position:absolute; inset:0; background-image:linear-gradient(rgba(224,120,0,0.05) 1px, transparent 1px), linear-gradient(90deg, rgba(224,120,0,0.05) 1px, transparent 1px); background-size:60px 60px; mask-image:radial-gradient(ellipse 80% 70% at 65% 30%, black 0%, transparent 70%); }
        .bg-shape { position:absolute; border:1px solid rgba(224,120,0,0.14); border-radius:3px; animation:shf ease-in-out infinite; }
        .bg-shape:nth-child(1){ width:70px; height:70px; right:12%; top:8%; transform:rotate(18deg); animation-duration:22s; }
        .bg-shape:nth-child(2){ width:42px; height:42px; right:4%; top:54%; border-radius:50%; border-color:rgba(255,149,0,0.12); animation-duration:28s; animation-delay:-9s; }
        .bg-shape:nth-child(3){ width:105px; height:105px; right:28%; top:16%; border-radius:50%; border-color:rgba(224,120,0,0.06); animation-duration:34s; animation-delay:-16s; }
        ...
        .bg-shape:nth-child(5){ width:56px; height:56px; right:50%; top:64%; border-radius:50%; border-color:rgba(255,215,64,0.07); animation-duration:25s; animation-delay:-13s; }
        @keyframes shf { 0%,100%{transform:translateY(0)} 50%{transform:translateY(-24px)} }

        /* NAV */
        .nav { position:fixed; top:0; left:0; right:0; z-index:200; display:flex; align-items:center; justify-content:space-between; padding:0 52px; height:70px; background:rgba(10,6,0,0.85); backdrop-filter:blur(22px); border-bottom:1px solid var(--border); }
        ...
        .nav-logo-txt span { background:linear-gradient(135deg,#FFB74D,#FFD740); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
        ...
        .btn-cta { padding:8px 20px; border-radius:6px; font-size:13px; font-weight:700; border:none; background:linear-gradient(135deg,var(--blue),var(--blue-bright)); color:#fff; cursor:pointer; font-family:'Syne',sans-serif; letter-spacing:0.03em; transition:all 0.2s; text-decoration:none; display:inline-flex; align-items:center; box-shadow:0 4px 20px rgba(224,120,0,0.35); }
        .btn-cta:hover { box-shadow:0 6px 28px rgba(224,120,0,0.5); transform:translateY(-1px); }
        ...
        .hero-title .grad { background:linear-gradient(135deg,#FFB74D 0%,#FFD740 60%,#FFB74D 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-size:200%; animation:shimmer 4s ease-in-out infinite alternate; }
        ...
        .btn-hero { padding:14px 30px; border-radius:8px; font-size:15px; font-weight:700; border:none; background:linear-gradient(135deg,#E07800,#FF9500); color:#fff; cursor:pointer; font-family:'Bricolage Grotesque',sans-serif; letter-spacing:-0.01em; transition:all 0.25s; text-decoration:none; display:inline-flex; align-items:center; gap:8px; box-shadow:0 8px 30px rgba(224,120,0,0.4); }
        .btn-hero:hover { box-shadow:0 12px 44px rgba(224,120,0,0.5); transform:translateY(-2px); }
        .btn-hero-ghost { padding:13px 30px; border-radius:8px; font-size:15px; font-weight:600; border:1px solid rgba(224,120,0,0.35); color:var(--text); background:transparent; cursor:pointer; font-family:'Bricolage Grotesque',sans-serif; transition:all 0.25s; text-decoration:none; display:inline-flex; align-items:center; gap:8px; }
        ...
        .badge-b { background:rgba(224,120,0,0.1); border:1px solid rgba(224,120,0,0.25); color:var(--blue-light); }
        ...
        .hg-glow { position:absolute; width:320px; height:320px; border-radius:50%; background:radial-gradient(circle, rgba(224,120,0,0.22) 0%, rgba(255,215,64,0.07) 50%, transparent 70%); filter:blur(28px); animation:hgGlow 4s ease-in-out infinite; }
        @keyframes hgGlow { 0%,100%{transform:scale(1);opacity:.9} 50%{transform:scale(1.1);opacity:1} }
        .hg-ring { position:absolute; top:50%; left:50%; width:340px; height:340px; border-radius:50%; border:1px solid rgba(224,120,0,0.18); transform:translate(-50%,-50%); animation:rRot 18s linear infinite; }
        .hg-ring2 { position:absolute; top:50%; left:50%; width:430px; height:280px; border-radius:50%; border:1px dashed rgba(255,215,64,0.1); transform:translate(-50%,-50%) rotate(-20deg); animation:rRot 28s linear infinite reverse; }
        @keyframes rRot { from{transform:translate(-50%,-50%) rotate(0deg)} to{transform:translate(-50%,-50%) rotate(360deg)} }

        /* Invoice card */
        .hg-card { position:absolute; top:50%; left:50%; transform:translate(-50%,-50%); width:268px; background:rgba(20,10,0,0.94); border:1px solid rgba(224,120,0,0.35); border-radius:14px; padding:20px 18px; box-shadow:0 20px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(224,120,0,0.1); backdrop-filter:blur(12px); animation:cardF 5s ease-in-out infinite; }
        ...
        .hg-row { display:flex; justify-content:space-between; padding:5px 0; border-bottom:1px solid rgba(224,120,0,0.08); }
        .hg-lbl { font-size:10px; color:var(--muted); }
        .hg-val { font-size:10px; color:var(--text); font-family:'DM Mono',monospace; }
        .hg-div { border:none; border-top:1px solid rgba(224,120,0,0.2); margin:8px 0; }
        ...
        .hg-qr { display:flex; align-items:center; gap:9px; margin-top:12px; padding-top:10px; border-top:1px solid rgba(224,120,0,0.1); }
        .hg-qr-box { width:34px; height:34px; background:rgba(224,120,0,0.08); border:1px solid rgba(224,120,0,0.22); border-radius:4px; display:flex; align-items:center; justify-content:center; font-size:18px; flex-shrink:0; }
        ...
        .hg-tab.active { background:rgba(224,120,0,0.18); border:1px solid rgba(224,120,0,0.4); color:var(--blue-light); }
        ...
        .hg-pill { position:absolute; background:rgba(20,10,0,0.92); border:1px solid rgba(224,120,0,0.28); border-radius:8px; padding:8px 12px; backdrop-filter:blur(8px); white-space:nowrap; box-shadow:0 8px 24px rgba(0,0,0,0.3); }
        ...
        .hg-bar { width:10px; border-radius:3px 3px 0 0; background:linear-gradient(to top, rgba(224,120,0,.65), rgba(255,215,64,.45)); animation:bP ease-in-out infinite; }
        ...
        /* STATS */
        .stats { padding:52px 8vw; background:rgba(224,120,0,0.04); border-top:1px solid var(--border); border-bottom:1px solid var(--border); display:grid; grid-template-columns:repeat(auto-fit,minmax(150px,1fr)); gap:32px; position:relative; z-index:1; }
        ...
        .inv-section { padding:92px 8vw; background:rgba(224,120,0,0.025); border-top:1px solid var(--border); border-bottom:1px solid var(--border); position:relative; z-index:1; }
        ...
        .inv-tab.on { background:rgba(224,120,0,0.16); border:1px solid rgba(224,120,0,0.4); color:var(--blue-light); }
        .inv-tab.off { background:var(--surface); border:1px solid var(--border-b); color:var(--muted); }

        /* A4 invoice mock */
        .inv-a4 { background:rgba(20,10,0,0.92); border:1px solid rgba(224,120,0,0.28); border-radius:10px; padding:22px 20px; font-family:'DM Mono',monospace; position:relative; overflow:hidden; }
        ...
        .inv-a4-bname span { background:linear-gradient(135deg,#FFB74D,#FFD740); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
        ...
        .inv-a4-table-head { display:grid; grid-template-columns:1fr auto auto; gap:8px; padding:5px 0; border-bottom:1px solid rgba(224,120,0,0.22); margin-bottom:4px; }
        .inv-a4-th { font-size:8px; color:var(--muted); letter-spacing:0.1em; text-transform:uppercase; }
        .inv-a4-tr { display:grid; grid-template-columns:1fr auto auto; gap:8px; padding:5px 0; border-bottom:1px solid rgba(224,120,0,0.07); }
        ...
        .inv-a4-footer { display:flex; justify-content:space-between; align-items:center; margin-top:14px; padding-top:12px; border-top:1px solid rgba(224,120,0,0.14); }
        .inv-a4-zimra { display:flex; align-items:center; gap:6px; }
        .inv-a4-qr { width:28px; height:28px; background:rgba(224,120,0,0.1); border:1px solid rgba(224,120,0,0.22); border-radius:3px; display:flex; align-items:center; justify-content:center; font-size:14px; }
        ...
        /* Receipt mock */
        .inv-receipt { background:rgba(20,10,0,0.92); border:1px solid rgba(224,120,0,0.28); border-radius:10px; padding:18px 16px; font-family:'DM Mono',monospace; max-width:220px; margin:0 auto; position:relative; }
        ...
        .inv-rdivider { border:none; border-top:1px dashed rgba(224,120,0,0.22); margin:8px 0; }
        ...
        .inv-rtotal { display:flex; justify-content:space-between; padding:6px 0; border-top:2px solid rgba(224,120,0,0.22); margin-top:4px; }
        ...
        .inv-point-ico { width:38px; height:38px; flex-shrink:0; border-radius:9px; background:var(--blue-dim); border:1px solid rgba(224,120,0,0.22); display:flex; align-items:center; justify-content:center; font-size:18px; }
        .inv-point-txt { }
        ...
        .feat-card:hover { background:rgba(224,120,0,0.06); }
        ...
        .feat-icon { width:42px; height:42px; border-radius:9px; background:var(--blue-dim); border:1px solid rgba(224,120,0,0.22); display:flex; align-items:center; justify-content:center; font-size:19px; margin-bottom:18px; }
        ...
        /* POS */
        .pos-section { padding:92px 8vw; background:rgba(224,120,0,0.02); border-top:1px solid var(--border); border-bottom:1px solid var(--border); position:relative; z-index:1; }
        ...
        .pos-mock { background:rgba(20,10,0,0.92); border:1px solid rgba(224,120,0,0.26); border-radius:12px; padding:24px; font-family:'DM Mono',monospace; }
        ...
        .pos-status { padding:3px 9px; border-radius:4px; background:rgba(224,120,0,0.1); border:1px solid rgba(224,120,0,0.24); font-size:9px; color:var(--blue-light); letter-spacing:0.1em; }
        .pos-item { display:flex; justify-content:space-between; padding:8px 0; border-bottom:1px solid rgba(224,120,0,0.08); color:var(--muted); font-size:12px; }
        .pos-total { display:flex; justify-content:space-between; padding:12px 0 0; border-top:2px solid rgba(224,120,0,0.22); margin-top:4px; }
        ...
        .pos-btn { width:100%; margin-top:12px; padding:11px; border-radius:6px; background:linear-gradient(135deg,var(--blue),var(--blue-bright)); color:#fff; font-family:'Bricolage Grotesque',sans-serif; font-weight:800; font-size:12px; border:none; cursor:pointer; box-shadow:0 4px 16px rgba(224,120,0,0.35); }
        ...
        .step-n { width:70px; height:70px; border-radius:50%; margin:0 auto 22px; background:linear-gradient(135deg, rgba(224,120,0,0.14), rgba(255,215,64,0.08)); border:2px solid rgba(224,120,0,0.28); display:flex; align-items:center; justify-content:center; font-family:'Bricolage Grotesque',sans-serif; font-size:22px; font-weight:800; color:var(--blue-light); }
        ...
        .price-card.hot { background:rgba(224,120,0,0.07); border-color:rgba(224,120,0,0.36); }
        ...
        .btn-plan { width:100%; padding:12px; border-radius:6px; font-size:13px; font-weight:700; font-family:'Bricolage Grotesque',sans-serif; cursor:pointer; border:1px solid rgba(224,120,0,0.26); background:transparent; color:var(--text); transition:all 0.22s; letter-spacing:-0.01em; }
        .btn-plan.hot-btn { background:linear-gradient(135deg,var(--blue),var(--blue-bright)); color:#fff; border-color:transparent; box-shadow:0 4px 18px rgba(224,120,0,0.35); }
        .btn-plan:hover { border-color:var(--blue-bright); color:var(--blue-light); }
        .btn-plan.hot-btn:hover { box-shadow:0 6px 26px rgba(224,120,0,0.5); transform:translateY(-1px); }
        ...
        .testi-av { width:36px; height:36px; border-radius:50%; flex-shrink:0; background:var(--blue-dim); border:2px solid rgba(224,120,0,0.28); display:flex; align-items:center; justify-content:center; font-family:'Bricolage Grotesque',sans-serif; font-weight:800; color:var(--blue-light); font-size:13px; }
        ...
        /* CTA */
        .cta-band { padding:104px 8vw; text-align:center; position:relative; z-index:1; background:linear-gradient(180deg, transparent 0%, rgba(224,120,0,0.05) 50%, transparent 100%); border-top:1px solid var(--border); }
        ...
        .cta-title .grad { background:linear-gradient(135deg,#FFB74D,#FFD740); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
        ...
        .footer-logo-txt span { background:linear-gradient(135deg,#FFB74D,#FFD740); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
        ...
        .footer-bottom { display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px; padding-top:18px; border-top:1px solid rgba(224,120,0,0.08); }
      ...
      `}</style>
            ...
            <div style={{ position: "relative", zIndex: 1 }}>
                ...
                {/* FAQ */}
                <section className="section" id="faq" style={{ background: "rgba(224,120,0,0.025)", borderTop: "1px solid var(--border)" }}>
