...
export function RecipeManager({ productId, companyId }: Props) {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Recipe / BOM Saved",
        description:
          "Ingredient and source-stock deduction setup updated successfully.",
      });
    },
  });
  ...
  const addItem = (product: any) => {
    if (items.some((item) => item.ingredientId === product.id)) {
      toast({
        title: "Already added",
        description:
          "This ingredient or source stock item is already selected.",
        variant: "destructive",
      });
      return;
    }
            ...
            <ScrollArea className="h-[400px]">
              {items.length === 0 ? (
                <div className="p-12 text-center text-slate-400 font-medium">
                  No ingredients or source stock added yet. Search and add from
                  the right panel.
                </div>
              ) : (
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-24">
                          <Input
                            ...
                            className="h-9 text-right font-black rounded-lg border-slate-200 "
                          />
                        </div>
                        <div className="w-12 text-xs font-bold text-slate-400 uppercase">
                          {item.unit}
                        </div>
                        <Button
                          variant="ghost"
      ...
      <div className="space-y-4">
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="pl-9 h-11 rounded-xl bg-slate-100 border-none "
          />
        </div>
          ...
          <div className="space-y-2">
            ...
            {ingredients.length === 0 && (
              <div className="p-8 text-center text-slate-400 text-xs italic">
                {searchQuery
                  ? "No matching input stock"
                  : "No products marked as ingredient/source stock found."}
              </div>
            )}
          </div>
        </ScrollArea>
        <p className="text-[9px] text-slate-400 px-2 italic bg-slate-50 p-2 rounded-lg leading-relaxed">
          <b>Tip:</b> Recipes/BOM deduct inputs at the time of sale. For meat,
          mark the whole cut as Ingredient / Source Stock, then set each
          sellable cut to consume it. Production is different: use production
          when you know the output quantities and want to convert stock before
          selling.
        </p>
      </div>
    </div>
