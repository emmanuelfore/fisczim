...
export function Receipt48({
  ...
  const paperSize =
    paperSizeProp || (company.posSettings?.paperSize as string) || "80mm";
  ...
  const isCreditNote =
    invoice.transactionType === "CreditNote" || invoice.type === "credit_note";
  const isDebitNote =
    invoice.transactionType === "DebitNote" || invoice.type === "debit_note";
  ...
  return (
    <div
      ...
      className={`bg-white p-2 text-black font-mono text-[10px] leading-tight receipt-content ${isA4 ? "mx-auto" : ""}`}
    >
      {/* [1] Logo */}
      {company.logoUrl && (
      ...
      {/* [2] Taxpayer Name */}
      <h1 className="text-center font-bold uppercase text-xs mb-1">
        {company.name}
      </h1>
      ...
      <div className="mb-2 pb-2 border-b border-dashed border-black">
        ...
        <div className="flex justify-between">
          ...
          <span>
            {company.fdmsDeviceSerialNo || company.deviceSerialNo || "---"}
          </span>
        </div>
        <div className="flex justify-between">
          <span>Device ID:</span>
        ...
        {(isCreditNote || isDebitNote) &&
          (invoice.creditNote ||
            ...
            <div className="mt-1 pt-1 border-t border-dotted border-black text-[9px]">
              ...
              <p>
                Inv No:{" "}
                {invoice.creditNote?.receiptGlobalNo ||
                  originalInvoice?.receiptGlobalNo ||
                  invoice.originalInvoiceNumber}
              </p>
              <p>
                Device ID:{" "}
                {invoice.creditNote?.deviceID ||
                  originalInvoice?.fdmsDeviceId ||
                  company.fdmsDeviceId}
              </p>
              <p>
                Date:{" "}
                {invoice.creditNote?.receiptDate ||
                  (originalInvoice?.issueDate &&
                    format(new Date(originalInvoice.issueDate), "dd/MM/yy"))}
              </p>
            </div>
          )}
      </div>
      ...
      <div className="mb-2 pb-2 border-b border-dashed border-black">
        <div className="flex justify-between text-xs font-bold">
          ...
          <span>
            {Number(invoice.total || invoice.receiptTotal).toFixed(2)}
          </span>
        </div>
        <div className="flex justify-between mt-1 pt-1 border-t border-dotted border-black/20">
          ...
          <span>
            {Number(invoice.paymentAmount || invoice.total).toFixed(2)}
          </span>
        </div>
        <div className="flex justify-between">
      ...
      <div className="mb-2 pb-2 border-b border-dashed border-black">
        ...
        {invoice.exchangeRate && invoice.currency !== "USD" && (
          <p className="font-bold text-center mt-1">
            USD Equivalent: $
            {(Number(invoice.total) / Number(invoice.exchangeRate)).toFixed(2)}
          </p>
        )}
      </div>
      ...
      {isVatPayer && (
        <div className="mb-2 pb-2 border-b border-dashed border-black">
          ...
          {Object.values(taxGroups).map((group: any, i) => (
            <div
              key={i}
              className="mb-1 border-b border-dotted border-gray-300 pb-1 last:border-0"
            >
              <div className="flex justify-between uppercase text-[9px]">
                <span>
                  Tax Code {group.name} ({group.rate}%)
                </span>
              </div>
      ...
      <div className="flex flex-col items-center gap-2 mb-2">
        {(() => {
          ...
          const qrData =
            ...
            company.qrUrl ||
            (isSimulated
              ? "https://fdms.zimra.co.zw/verify/SIMULATION-ONLY"
              : "");
          ...
          return (
            <>
              {qrData && (
                <QRCodeSVG value={qrData} size={110} level="M" marginSize={1} />
              )}
