...
export function ExpenseDetailsReport({
  companyId,
  dateRange,
  search,
}: ReportProps) {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiFetch(
        buildUrl(companyId, "expense-details", dateRange),
      );
      if (!res.ok)
        throw new Error(`Failed to load expense details (${res.status})`);
      return res.json();
    },
    enabled: !!companyId,
  });
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <DetailCard
                ...
                value={selectedRow.paymentMethod}
              />
...
export function ExpensesByCategoryReport({
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiFetch(
        buildUrl(companyId, "expenses-by-category", dateRange),
      );
      if (!res.ok)
        throw new Error(`Failed to load expenses by category (${res.status})`);
      return res.json();
    },
    enabled: !!companyId,
  });
  ...
  return (
    <div className="flex h-full">
      <div className="w-1/2 border-r border-slate-100 overflow-auto">
        ...
        {filtered.length === 0 ? (
          <EmptyState message="No expense categories in this period" />
        ) : (
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-white border-b border-slate-100">
              <tr>
...
export function ExpensesByCustomerReport({
  companyId,
  dateRange,
  search,
}: ReportProps) {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiFetch(
        buildUrl(companyId, "expenses-by-customer", dateRange),
      );
      if (!res.ok)
        throw new Error(`Failed to load expenses by customer (${res.status})`);
      return res.json();
    },
    enabled: !!companyId,
  });
  ...
  return (
    <div className="flex h-full">
      <div className="w-1/2 border-r border-slate-100 overflow-auto">
        ...
        {filtered.length === 0 ? (
          <EmptyState message="No supplier expenses in this period" />
        ) : (
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-white border-b border-slate-100">
              <tr>
...
export function ExpensesByProjectReport({
  companyId,
  dateRange,
  search,
}: ReportProps) {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiFetch(
        buildUrl(companyId, "expenses-by-project", dateRange),
      );
      if (!res.ok)
        throw new Error(`Failed to load expenses by project (${res.status})`);
      return res.json();
    },
    enabled: !!companyId,
  });
  ...
  return (
    <div className="flex h-full">
      <div className="w-1/2 border-r border-slate-100 overflow-auto">
        ...
        {filtered.length === 0 ? (
          <EmptyState message="No project expenses in this period" />
        ) : (
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-white border-b border-slate-100">
              <tr>
...
export function BillableExpenseDetailsReport({
  companyId,
  dateRange,
  search,
}: ReportProps) {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiFetch(
        buildUrl(companyId, "billable-expense-details", dateRange),
      );
      if (!res.ok)
        throw new Error(
          `Failed to load billable expense details (${res.status})`,
        );
      return res.json();
    },
    enabled: !!companyId,
  ...
  return (
    <div className="flex h-full">
      <div className="w-1/2 border-r border-slate-100 overflow-auto">
        ...
        {filtered.length === 0 ? (
          <EmptyState message="No billable expenses in this period" />
        ) : (
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-white border-b border-slate-100">
              <tr>
