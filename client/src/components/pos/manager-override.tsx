...
export function ManagerOverride({
  ...
  description = "Enter PIN to proceed",
  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await apiFetch(
        `/api/companies/${companyId}/auth/verify-manager-pin`,
        {
          method: "POST",
      ...
      if (res.ok && data.authorized) {
        toast({
          title: "Authorized",
          description: `Approved by ${data.manager.name}`,
        });
      ...
      if (window.electronAPI) {
        try {
          const verified = await window.electronAPI.verifyManagerPin(
            pin,
            companyId,
          );
          if (verified) {
            toast({
              title: "Authorized",
              description: "Approved offline via cached PIN",
            });
          ...
          toast({
            title: "Error",
            description: "Offline verification failed",
            variant: "destructive",
          });
        }
      } else {
              ...
              <Button
                ...
                className={cn(
                  ...
                  btn === "DEL"
                    ...
                    : btn === "CLEAR"
                      ? "text-orange-500 text-xs font-bold uppercase hover:bg-orange-50"
