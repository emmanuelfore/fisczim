...
type OpeningState = {
  ...
  customerSubledgerDocs?: Array<{
    customerId: number;
    customerName: string;
    invoiceId: number;
    amount: number;
  }>;
  supplierSubledgerDocs?: Array<{
    supplierId: number;
    supplierName: string;
...
export default function OpeningBalancesPage() {
  ...
  const [customers, setCustomers] = useState<SubledgerLine[]>([
    emptySubledgerLine(),
  ]);
  const [suppliers, setSuppliers] = useState<SubledgerLine[]>([
    emptySubledgerLine(),
  ]);
  ...
  const postMutation = useMutation({
    mutationFn: async () => {
      const payload = {
        ...
        customerBalances: customers
          .filter((row) => row.name && Number(row.amount || 0) > 0)
    ...
    onSuccess: () => {
      toast({
        ...
        description:
          "The opening entry is locked and available in the audit trail.",
      });
    ...
    <Layout hideHeaderTitle>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className=" text-slate-500">
              Post migration balances from Sage, Pastel, QuickBooks, or Excel.
            </p>
          </div>
          {opening?.locked ? (
        ...
        {opening?.locked ? (
          <Card>
            ...
            <CardContent className=" text-slate-600">
              ...
              {opening.date
                ? format(new Date(opening.date), "PPP")
                : "the selected date"}
              . Reverse journal #{opening.journalEntryId} from the audit trail
              to unlock and repost.
              <div className="mt-4 grid gap-3 md:grid-cols-2">
                <div className="rounded-lg border border-slate-100 bg-slate-50 p-3">
                  ...
                  <p className="text-xs text-slate-500">
                    {opening.customerSubledgerDocs?.length || 0} opening
                    customer invoice
                    {(opening.customerSubledgerDocs?.length || 0) === 1
                      ? ""
                      : "s"}{" "}
                    created.
                  </p>
                ...
                <div className="rounded-lg border border-slate-100 bg-slate-50 p-3">
                  ...
                  <p className="text-xs text-slate-500">
                    {opening.supplierSubledgerDocs?.length || 0} opening
                    supplier bill
                    {(opening.supplierSubledgerDocs?.length || 0) === 1
                      ? ""
                      : "s"}{" "}
                    created.
                  </p>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Scale className="h-5 w-5" /> Opening Trial Balance
            </CardTitle>
          </CardHeader>
                    ...
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        <Button
                          variant="ghost"
                          size="icon"
                          disabled={opening?.locked || lines.length <= 2}
                          onClick={() =>
                            setLines((current) =>
        ...
        <div className="grid gap-4 lg:grid-cols-3">
              ...
              <Card key={kind}>
                ...
                <CardContent className="space-y-3">
                  ...
                  <Button
                    ...
                    disabled={opening?.locked}
                    onClick={() =>
                      (kind === "customers" ? setCustomers : setSuppliers)(
                        (current) => [...current, emptySubledgerLine()],
                      )
                    }
                  >
                  ...
                  <p className="text-xs text-slate-500">
                    {kind === "customers"
                      ? "Each line creates an opening receivable invoice for aging and statements."
                      : "Each line creates an opening supplier bill for payables aging and supplier drilldown."}
                  </p>
                </CardContent>
              </Card>
            );
          ...
          <Card>
            <CardHeader>
              <CardTitle>Inventory Opening Value</CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              ...
              />
              <p className="text-xs text-slate-500">
                Posts to the inventory control account and balances through
                opening equity if needed.
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-5 md:flex-row md:items-center md:justify-between">
            <div className="">
              ...
              <span className="font-bold text-slate-800">
                Credit {netCredit.toFixed(2)}
              </span>
              <span className="mx-3 text-slate-300">|</span>
              <span className="text-slate-500">
                Difference {(netDebit - netCredit).toFixed(2)} will use opening
                equity.
              </span>
            </div>
            <Button
              disabled={opening?.locked || postMutation.isPending}
