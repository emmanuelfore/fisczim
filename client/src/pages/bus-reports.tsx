...
import { BarChart3, Bus, Clock, CreditCard, Download, Route, Ticket, UserRound, Users } from "lucide-react";
...
function TripPerformanceTable({ trips }: { trips: any[] }) {
  return (
    <Card className="border-none shadow-sm">
      <CardHeader className="pb-3"><CardTitle className="flex items-center gap-2 text-base"><Bus className="h-5 w-5 text-orange-500" />Trip Performance</CardTitle></CardHeader>
      <CardContent>
        <Table>
          <TableHeader>
            <TableRow>
          ...
          <TableBody>
            {trips.length === 0 ? (
              <TableRow><TableCell colSpan={8} className="h-24 text-center text-slate-500">No trips in this period.</TableCell></TableRow>
            ) : trips.slice(0, 100).map((trip) => (
              <TableRow key={trip.id}>
                <TableCell>
                  <div className="font-semibold text-slate-900">{trip.direction || trip.routeName || `Trip #${trip.id}`}</div>
                  <div className="text-xs text-slate-500">#{trip.id} - {trip.status || "unknown"} - {trip.tickets} tickets</div>
                </TableCell>
                <TableCell>{trip.scheduledDeparture ? format(new Date(trip.scheduledDeparture), "MMM d, HH:mm") : "-"}</TableCell>
...
function TripCashTable({ trips }: { trips: any[] }) {
  return (
    <Card className="border-none shadow-sm">
      <CardHeader className="pb-3"><CardTitle className="text-base">Trip Cash Split</CardTitle></CardHeader>
      <CardContent>
        <Table>
          <TableHeader><TableRow><TableHead>Trip</TableHead><TableHead className="text-right">Cash</TableHead><TableHead className="text-right">Non-Cash</TableHead><TableHead className="text-right">Total</TableHead></TableRow></TableHeader>
          <TableBody>
            {trips.length === 0 ? (
              <TableRow><TableCell colSpan={4} className="h-20 text-center text-slate-500">No trip cash data.</TableCell></TableRow>
            ) : trips.slice(0, 10).map((trip) => (
              <TableRow key={trip.id}>
                <TableCell className="font-semibold">{trip.direction || `Trip #${trip.id}`}</TableCell>
                <TableCell className="text-right">{money(trip.cashRevenue)}</TableCell>
                <TableCell className="text-right">{money(trip.nonCashRevenue)}</TableCell>
                <TableCell className="text-right font-bold">{money(trip.revenue)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
...
function RouteKpiTable({ rows }: { rows: any[] }) {
  return (
    <Card className="border-none shadow-sm">
      <CardHeader className="pb-3"><CardTitle className="text-base">Route Profitability Indicators</CardTitle></CardHeader>
      <CardContent>
        <Table>
          <TableHeader><TableRow><TableHead>Route</TableHead><TableHead className="text-right">Revenue / Km</TableHead><TableHead className="text-right">Passengers / Trip</TableHead><TableHead className="text-right">Occupancy</TableHead><TableHead className="text-right">Cancelled</TableHead><TableHead className="text-right">Revenue</TableHead></TableRow></TableHeader>
          <TableBody>
            {rows.length === 0 ? (
              <TableRow><TableCell colSpan={6} className="h-20 text-center text-slate-500">No route KPIs in this period.</TableCell></TableRow>
            ) : rows.slice(0, 20).map((row) => (
              <TableRow key={row.id}>
                <TableCell>
                  <div className="font-semibold">{row.label}</div>
                  <div className="text-xs text-slate-500">{row.trips} trips - {row.distanceKm || 0} km</div>
                </TableCell>
                <TableCell className="text-right">{row.revenuePerKm === null ? "-" : money(row.revenuePerKm)}</TableCell>
                <TableCell className="text-right">{Number(row.passengersPerTrip || 0).toFixed(1)}</TableCell>
                <TableCell className="text-right">{percent(row.occupancyRate)}</TableCell>
                <TableCell className="text-right">{row.cancelledTrips || 0}</TableCell>
                <TableCell className="text-right font-bold">{money(row.revenue)}</TableCell>
              </TableRow>
            ))}
          </TableBody>
...
function AuditTable({ data }: { data: any }) {
  ...
  return (
    <Card className="border-none shadow-sm">
      <CardHeader className="pb-3"><CardTitle className="text-base">Unsynced Ticket Audit</CardTitle></CardHeader>
      <CardContent>
        <Table>
          <TableHeader><TableRow><TableHead>Ticket</TableHead><TableHead>Trip</TableHead><TableHead>Time</TableHead><TableHead className="text-right">Amount</TableHead></TableRow></TableHeader>
          <TableBody>
            {rows.length === 0 ? (
              <TableRow><TableCell colSpan={4} className="h-20 text-center text-slate-500">No unsynced cloud tickets in this period.</TableCell></TableRow>
            ) : rows.slice(0, 50).map((ticket: any) => (
              <TableRow key={ticket.id}>
                <TableCell className="font-mono text-xs">{ticket.ticketNumber}</TableCell>
                <TableCell>{ticket.tripId ? `#${ticket.tripId}` : "-"}</TableCell>
...
function CashupReportTable({ cashup, variance }: { cashup: any; variance: any[] }) {
  return (
    <Card className="border-none shadow-sm">
      <CardHeader className="pb-3"><CardTitle className="text-base">Reconciled vs Unreconciled Cash</CardTitle></CardHeader>
      <CardContent>
        <div className="mb-4 grid gap-3 md:grid-cols-4">
          <div className="rounded-lg bg-slate-50 p-3"><p className="text-xs font-bold text-slate-500">Expected</p><p className="text-lg font-black">{money(cashup?.expectedCash || 0)}</p></div>
          <div className="rounded-lg bg-slate-50 p-3"><p className="text-xs font-bold text-slate-500">Received</p><p className="text-lg font-black">{money(cashup?.cashReceived || 0)}</p></div>
          <div className="rounded-lg bg-slate-50 p-3"><p className="text-xs font-bold text-slate-500">Variance</p><p className="text-lg font-black">{money(cashup?.variance || 0)}</p></div>
          <div className="rounded-lg bg-slate-50 p-3"><p className="text-xs font-bold text-slate-500">Open</p><p className="text-lg font-black">{cashup?.unreconciled || 0}</p></div>
        </div>
        <Table>
          <TableHeader><TableRow><TableHead>Conductor</TableHead><TableHead className="text-right">Expected</TableHead><TableHead className="text-right">Received</TableHead><TableHead className="text-right">Variance</TableHead><TableHead className="text-right">Exceptions</TableHead></TableRow></TableHeader>
          <TableBody>
            {variance.length === 0 ? (
              <TableRow><TableCell colSpan={5} className="h-20 text-center text-slate-500">No reconciliations in this period.</TableCell></TableRow>
            ) : variance.map((row: any) => (
              <TableRow key={row.id}>
                <TableCell className="font-semibold">{row.label}</TableCell>
                <TableCell className="text-right">{money(row.expectedCash)}</TableCell>
                <TableCell className="text-right">{money(row.cashReceived)}</TableCell>
                <TableCell className="text-right font-bold">{money(row.variance)}</TableCell>
...
export default function BusReportsPage() {
  ...
  const initialReportMode = useMemo<ReportMode>(() => {
    ...
    return mode === "range" || mode === "conductor" || mode === "trip" ? mode : "daily";
  }, []);
  ...
  const trips = useMemo(() => {
    ...
    if (reportMode !== "conductor" || conductorId === "all") return list;
    return list.filter((trip: any) => String(trip.conductorId) === String(conductorId));
  ...
  const byDirection = useMemo(() => summarize(tickets, (ticket) => ticket.direction || "Unknown", (ticket) => ticket.direction || "Unknown"), [tickets]);
  ...
  const averageOccupancy = trips.length ? trips.reduce((sum: number, trip: any) => sum + Number(trip.occupancyRate || 0), 0) / trips.length : null;
  const topTrip = trips.reduce((best: any, trip: any) => Number(trip.revenue || 0) > Number(best.revenue || 0) ? trip : best, { id: "", direction: "-", revenue: 0 });

  function exportCsv() {
    const headers = ["Ticket", "Trip", "Date", "Time", "Route", "Direction", "Conductor", "Vehicle", "Passengers", "Amount", "Payment"];
  ...
  return (
    <Layout>
        ...
        actions={
          <div className="flex flex-col gap-2 lg:flex-row lg:items-center">
            <Select value={reportMode} onValueChange={(value: ReportMode) => handleReportModeChange(value)}>
              ...
              <SelectContent><SelectItem value="daily">Daily Report</SelectItem><SelectItem value="range">Range Report</SelectItem><SelectItem value="trip">Trip Report</SelectItem><SelectItem value="conductor">Conductor Report</SelectItem></SelectContent>
            </Select>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-3 xl:grid-cols-5">
        <StatCard icon={Bus} label="Trips" value={isLoading ? "..." : trips.length} tone="blue" />
        <StatCard icon={Route} label="Completed" value={isLoading ? "..." : completedTrips} tone="emerald" />
        <StatCard icon={Users} label="Avg Load" value={isLoading ? "..." : percent(averageOccupancy)} tone="slate" />
        <StatCard icon={Ticket} label="Passengers / Trip" value={isLoading || trips.length === 0 ? "-" : (totals.passengers / trips.length).toFixed(1)} tone="orange" />
        <StatCard icon={BarChart3} label="Top Trip" value={topTrip.id ? money(topTrip.revenue) : "-"} tone="emerald" />
      </div>

      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        <MiniBars title={reportMode === "range" || reportMode === "trip" ? "Daily Revenue Trend" : "Revenue By Hour"} rows={reportMode === "range" || reportMode === "trip" ? byDay : byHour} />
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-2">
        ...
        <MiniBars title="Top Trips By Revenue" rows={trips.slice(0, 10).map((trip: any) => ({ id: String(trip.id), label: trip.direction || `Trip #${trip.id}`, revenue: Number(trip.revenue || 0), tickets: Number(trip.tickets || 0) }))} />
      </div>
      ...
      <Card className="mt-4 border-none shadow-sm">
        ...
        <CardContent>
          <Table>
            <TableHeader><TableRow><TableHead>Ticket</TableHead><TableHead>Trip</TableHead><TableHead>Direction</TableHead><TableHead>Conductor</TableHead><TableHead>Vehicle</TableHead><TableHead>Payment</TableHead><TableHead>Time</TableHead><TableHead className="text-right">Amount</TableHead></TableRow></TableHeader>
            <TableBody>
              {tickets.length === 0 ? (
                <TableRow><TableCell colSpan={8} className="h-24 text-center text-slate-500">No tickets in this period.</TableCell></TableRow>
              ) : tickets.slice(0, 200).map((ticket: any) => (
                <TableRow key={ticket.id}>
                  <TableCell className="font-mono text-xs">{ticket.ticketNumber}</TableCell>
                  <TableCell>{ticket.tripId ? `#${ticket.tripId}` : "-"}</TableCell>
                  <TableCell>{ticket.direction || ticket.routeName || "-"}</TableCell>
