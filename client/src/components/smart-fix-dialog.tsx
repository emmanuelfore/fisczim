...
export function SmartFixDialog({
  ...
  const isDayClosed =
    errorMessage.includes("FiscalDayClosed") ||
    errorMessage.includes("Fiscal Day Closed") ||
    errorMessage.includes("fiscal day is closed");
  const isOffline =
    errorMessage.includes("offline") ||
    errorMessage.includes("Network Error") ||
    errorMessage.includes("fetch failed");
  const isCertError =
    errorMessage.includes("certificate") || errorMessage.includes("keys");
  ...
  if (isDayClosed) {
    ...
    description =
      "The device appears to be offline or unreachable. The invoice has been queued safely and will be processed when the connection is restored.";
    action = (
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <div
            className={`flex items-center gap-2 mb-2 ${isDayClosed ? "text-slate-600" : "text-amber-600"}`}
          >
            {isDayClosed ? (
