...
export function BusTicketingSettings({ formData, setFormData }: Props) {
  ...
  const grouped = BUS_FEATURES.reduce(
    (acc, feature) => {
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <h2 className="text-lg font-bold text-[#0F172A]">
                Bus Ticketing Module
              </h2>
              <p className="mt-1 max-w-2xl  text-[#64748B]">
                Choose which bus-ticketing tools appear in the web admin and
                conductor APK.
              </p>
            </div>
          </div>
      ...
      {Object.entries(grouped).map(([group, features]) => (
        <Card key={group} className="border-[#E5E7EB] shadow-sm">
          <CardHeader className="pb-3">
            ...
            <CardDescription>
              Turn off anything you do not want users to see yet.
            </CardDescription>
          </CardHeader>
