...
export default function SubscriptionPage() {
  ...
  useEffect(() => {
    if (activeCompany) {
      ...
      setManualMacAddress(
        activeCompany.registeredMacAddress || macAddress || "",
      );
    }
    ...
    try {
      ...
      if (data.macAddresses && data.macAddresses.length > 0) {
        ...
        toast({
          ...
          description: `Automatically identified machine: ${data.macAddresses[0]}`,
        });
      } else {
        toast({
          title: "Detection Failed",
          description: "Could not identify any physical network interfaces.",
          variant: "destructive",
        });
      }
    } catch (err: any) {
      toast({
        title: "Error",
        description: "Failed to communicate with local hardware service.",
        variant: "destructive",
      });
    }
  };
  ...
  const initiateMutation = useMutation({
    mutationFn: async () => {
      const res = await apiFetch(
        `/api/companies/${activeCompany?.id}/subscriptions/initiate`,
        {
          method: "POST",
          body: JSON.stringify({
            amount: 150,
  ...
  const manualActivateMutation = useMutation({
    ...
    onError: (err: any) => {
      toast({
        title: "Error",
        description: err.message || "Failed to activate subscription manually",
        variant: "destructive",
      });
    },
  });
  ...
  const isThisDeviceSubscribed = activeSubs.some(
    (s: any) => s.deviceMacAddress === macAddress,
  );
    ...
    <Layout>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className=" font-bold text-slate-900 font-mono">
                            {sub.deviceMacAddress}
                          </div>
                          <div className="text-[11px] text-slate-500">
                            Serial: {sub.deviceSerialNo}
                          </div>
                        </div>
                      </div>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[10px] text-slate-400">
                    Enter the MAC address of the device you want to license.
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 bg-amber-50 border border-amber-100 rounded-lg flex gap-3">
                ...
                <div className=" text-amber-800">
                  <strong>One-to-One Binding</strong>: Each physical device
                  requires its own **$150.00 USD / year** subscription. MAC
                  addresses are used to verify the hardware identity.
                </div>
              </div>
              ...
              <Button
                ...
                disabled={
                  initiateMutation.isPending || !macAddress || !targetSerialNo
                }
              >
                ...
                Pay $150.00 / year for{" "}
                {targetSerialNo || macAddress || "New Device"}
              </Button>
        ...
        <div className="space-y-6">
          <Card className="border-none shadow-sm">
            <CardHeader>
              <CardTitle className="">Multi-Device Support</CardTitle>
            </CardHeader>
            ...
            <Card className="border-none shadow-sm bg-amber-50 border-amber-200">
              <CardHeader>
                ...
                <CardDescription className="text-amber-800/70">
                  Record a cash payment and activate immediately.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <Label htmlFor="manualSerial" className="text-amber-900">
                      Device Serial
                    </Label>
                    <Input
                      id="manualSerial"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="manualMac" className="text-amber-900">
                      MAC Address
                    </Label>
                    <Input
                      id="manualMac"
                ...
                <div className="space-y-2">
                  <Label htmlFor="manualAmount" className="text-amber-900">
                    Amount (USD)
                  </Label>
                  <Input
                    id="manualAmount"
                ...
                <div className="space-y-2">
                  <Label htmlFor="manualNotes" className="text-amber-900">
                    Internal Notes
                  </Label>
                  <Input
                    id="manualNotes"
