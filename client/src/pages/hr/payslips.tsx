...
export default function HRPayslips() {
  ...
  const handlePrint = (payslip: any) => {
    ...
    let snapshot = null;
    try {
      snapshot = typeof payslip.runData.snapshotData === 'string' 
        ? JSON.parse(payslip.runData.snapshotData) 
        : payslip.runData.snapshotData;
    } catch (e) {}
        ...
        <body>
          ...
          <div class="grid">
            <div class="col">
              ...
              <div class="value">${payslip.employee.firstName} ${payslip.employee.lastName}</div>
              <div class="label">Employee ID</div>
              <div class="value">${payslip.employee.employeeNumber}</div>
            </div>
            <div class="col">
              <div class="label">National ID</div>
              <div class="value">${payslip.employee.nationalId || '-'}</div>
              <div class="label">Bank Account</div>
              <div class="value">${payslip.employee.bankName || '-'} (${payslip.employee.bankAccountNumber || '-'})</div>
            </div>
          </div>
          ...
          <table>
            ...
            <tbody>
              <tr>
                <td>Basic Salary</td>
                <td class="amount">$${parseFloat(payslip.runData.basicSalary).toFixed(2)}</td>
              </tr>
              ${parseFloat(payslip.runData.grossSalary) > parseFloat(payslip.runData.basicSalary) ? `
              <tr>
                <td>Allowances</td>
                <td class="amount">$${(parseFloat(payslip.runData.grossSalary) - parseFloat(payslip.runData.basicSalary)).toFixed(2)}</td>
              </tr>` : ''}
              <tr class="total-row">
                <td>Gross Earnings</td>
                <td class="amount">$${parseFloat(payslip.runData.grossSalary).toFixed(2)}</td>
              </tr>
            </tbody>
          </table>
          ...
          <table>
            ...
            <tbody>
              <tr>
                <td>PAYE (ZIMRA)</td>
                <td class="amount">$${parseFloat(payslip.runData.paye).toFixed(2)}</td>
              </tr>
              <tr>
                <td>NSSA</td>
                <td class="amount">$${parseFloat(payslip.runData.nssaEmployee).toFixed(2)}</td>
              </tr>
              <tr>
                <td>AIDS Levy</td>
                <td class="amount">$${parseFloat(payslip.runData.aidsLevy).toFixed(2)}</td>
              </tr>
              <tr class="total-row">
                <td>Total Deductions</td>
                <td class="amount">$${(parseFloat(payslip.runData.paye) + parseFloat(payslip.runData.nssaEmployee) + parseFloat(payslip.runData.aidsLevy)).toFixed(2)}</td>
              </tr>
            </tbody>
          </table>

          <div class="net-pay">
            Net Pay: $${parseFloat(payslip.runData.netSalary).toFixed(2)}
          </div>
        </body>
      </html>
    `;
