  ...
  const { mutate: clearData, isPending } = useMutation({
    mutationFn: async () => {
      const res = await apiFetch(
        `/api/companies/${company.id}/maintenance/clear-data`,
        {
          method: "POST",
        },
      );
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className=" text-muted-foreground">
          System maintenance tools and data clearing options
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <CardContent className="space-y-6 pt-2">
            <div className="bg-white p-4 rounded-2xl border border-red-100 shadow-sm space-y-3">
              ...
              <div className="pt-2">
                <p className="text-[11px] font-bold text-red-600 uppercase tracking-wider">
                  Caution: This action cannot be undone. Always backup your data
                  first.
                </p>
              </div>
            </div>
            ...
            <AlertDialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <AlertDialogTrigger asChild>
                <Button
                  variant="destructive"
                  className="w-full h-12 rounded-2xl font-black gap-2 shadow-lg shadow-red-200 active:scale-95 transition-all"
                >
              ...
              <AlertDialogContent className="rounded-[2rem] border-none shadow-2xl p-8 max-w-md">
                <AlertDialogHeader>
                  ...
                  <AlertDialogDescription className="text-slate-500 font-medium py-4">
                    This will permanently destroy all financial records for{" "}
                    <span className="font-bold text-slate-900">
                      {company.name}
                    </span>
                    . The dashboard will be completely reset. This action is
                    irreversible.
                  </AlertDialogDescription>
                </AlertDialogHeader>
                ...
                <AlertDialogFooter className="gap-3">
                  <AlertDialogCancel className="h-12 rounded-xl font-bold border-slate-200">
                    Cancel
                  </AlertDialogCancel>
                  <AlertDialogAction
        ...
        <Card className="card-depth border-none opacity-50 grayscale cursor-not-allowed">
          <CardHeader>
            ...
            <CardDescription>
              Export/Import full system snapshots
            </CardDescription>
          </CardHeader>
