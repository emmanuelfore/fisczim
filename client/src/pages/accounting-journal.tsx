...
import { Plus, History, User, Calendar, Trash2, Save, Send } from "lucide-react";
...
type FullJournalEntry = JournalEntry & {
  lines?: Array<LedgerEntry & { account?: Account; accountCode?: string; accountName?: string }>;
  ledgerEntries?: Array<LedgerEntry & { account?: Account; accountCode?: string; accountName?: string }>;
};
...
type JournalDraft = {
  ...
  referenceId?: string;
  status: string;
  lines: Array<{ id?: number; accountId: number; accountCode: string; accountName: string; type: "DEBIT" | "CREDIT"; amount: string }>;
};
...
const emptyVoucherLine = (): VoucherLine => ({ accountId: "", debit: "", credit: "", memo: "" });
...
export default function AccountingJournalPage() {
  ...
  const buildPayload = () => {
      ...
      const lines = voucher.lines.flatMap((line) => {
        ...
        if (!line.accountId && (debit > 0 || credit > 0)) throw new Error("Each amount needs an account");
      ...
      if (!isBalanced) throw new Error(`Journal is out of balance. Debits: ${totals.debit.toFixed(2)}, Credits: ${totals.credit.toFixed(2)}`);
      return {
  ...
  const saveDraftMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", "/api/accounting/journal-drafts", buildPayload());
      return res.json();
    },
    onSuccess: () => {
      ...
      resetVoucher();
      queryClient.invalidateQueries({ queryKey: ["/api/accounting/journal-drafts"] });
    },
    onError: (error: any) => {
  ...
  const postVoucherMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("POST", "/api/accounting/journal", buildPayload());
      return res.json();
    },
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast({ title: "Could not post voucher", description: error.message, variant: "destructive" });
    },
  });

  const postDraftMutation = useMutation({
    mutationFn: async (draftId: number) => {
      const res = await apiRequest("POST", `/api/accounting/journal-drafts/${draftId}/post`);
      return res.json();
    },
    onSuccess: () => {
    ...
    onError: (error: any) => {
      toast({ title: "Could not post draft", description: error.message, variant: "destructive" });
    },
  });
            ...
            <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-5 pt-2">
                ...
                <div className="space-y-2">
                  <Label>Date</Label>
                  <Input type="date" value={voucher.entryDate} onChange={(e) => setVoucher({ ...voucher, entryDate: e.target.value })} />
                </div>
                <div className="space-y-2">
                  <Label>Reference</Label>
                  <Input value={voucher.referenceId} placeholder="JV-0001" onChange={(e) => setVoucher({ ...voucher, referenceId: e.target.value })} />
                </div>
                <div className="space-y-2 md:col-span-2">
                  <Label>Description</Label>
                  <Input value={voucher.description} placeholder="Narrative" onChange={(e) => setVoucher({ ...voucher, description: e.target.value })} />
                </div>
                </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                        ...
                        <TableRow key={index}>
                          <TableCell>
                            <Select
                              ...
                              onValueChange={(value) => setVoucher((current) => ({
                                ...current,
                                lines: current.lines.map((item, lineIndex) => lineIndex === index ? { ...item, accountId: value } : item),
                              }))}
                            >
                          ...
                          <TableCell>
                            <Input
                              ...
                              onChange={(e) => setVoucher((current) => ({
                                ...current,
                                lines: current.lines.map((item, lineIndex) => lineIndex === index ? { ...item, memo: e.target.value } : item),
                              }))}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              ...
                              onChange={(e) => setVoucher((current) => ({
                                ...current,
                                lines: current.lines.map((item, lineIndex) => lineIndex === index ? { ...item, debit: e.target.value, credit: e.target.value ? "" : item.credit } : item),
                              }))}
                            />
                          </TableCell>
                          <TableCell>
                            <Input
                              ...
                              onChange={(e) => setVoucher((current) => ({
                                ...current,
                                lines: current.lines.map((item, lineIndex) => lineIndex === index ? { ...item, credit: e.target.value, debit: e.target.value ? "" : item.debit } : item),
                              }))}
                            />
                          </TableCell>
                          <TableCell>
                            <Button
                              ...
                              onClick={() => setVoucher((current) => ({ ...current, lines: current.lines.filter((_, lineIndex) => lineIndex !== index) }))}
                            >
                      ...
                      <TableRow className="bg-slate-50">
                        <TableCell colSpan={2}>
                          <Button variant="outline" size="sm" onClick={() => setVoucher((current) => ({ ...current, lines: [...current.lines, emptyVoucherLine()] }))}>
                            <Plus className="h-4 w-4 mr-2" /> Add Line
                          </Button>
                        </TableCell>
                        <TableCell className="text-right font-black">{totals.debit.toLocaleString(undefined, { minimumFractionDigits: 2 })}</TableCell>
                        <TableCell className="text-right font-black">{totals.credit.toLocaleString(undefined, { minimumFractionDigits: 2 })}</TableCell>
                        <TableCell />
                      </TableRow>
                    </TableBody>
                  </Table>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-3">
                  <Badge variant="outline" className={cn("w-fit font-bold", isBalanced ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-rose-50 text-rose-700 border-rose-200")}>
                    {isBalanced ? "Balanced" : `Difference ${(totals.debit - totals.credit).toLocaleString(undefined, { minimumFractionDigits: 2 })}`}
                  </Badge>
                  <div className="flex justify-end gap-2">
                    <Button variant="outline" onClick={() => saveDraftMutation.mutate()} disabled={saveDraftMutation.isPending}>
                      <Save className="h-4 w-4 mr-2" /> {saveDraftMutation.isPending ? "Saving..." : "Save Draft"}
                    </Button>
                    <Button onClick={() => postVoucherMutation.mutate()} disabled={postVoucherMutation.isPending || !isBalanced}>
                      <Send className="h-4 w-4 mr-2" /> {postVoucherMutation.isPending ? "Posting..." : "Post Now"}
                    </Button>
                  </div>
                </div>
              </div>
        ...
        {drafts?.some((draft) => draft.status === "DRAFT") && (
          <Card className="border-amber-200 bg-amber-50/40 rounded-2xl">
            <CardHeader className="px-6 py-4">
              <CardTitle className="text-base font-bold text-amber-900">Draft Journal Batches</CardTitle>
            </CardHeader>
              ...
              <Table>
                ...
                <TableBody>
                  {drafts.filter((draft) => draft.status === "DRAFT").map((draft) => {
                    const debit = draft.lines.filter((line) => line.type === "DEBIT").reduce((sum, line) => sum + Number(line.amount), 0);
                    const credit = draft.lines.filter((line) => line.type === "CREDIT").reduce((sum, line) => sum + Number(line.amount), 0);
                    return (
                      <TableRow key={draft.id}>
                        <TableCell className="pl-6 text-sm font-medium">{format(new Date(draft.entryDate), "dd MMM yyyy")}</TableCell>
                        <TableCell className="font-bold text-slate-800">{draft.description}</TableCell>
                        <TableCell className="text-xs text-slate-500">{draft.referenceType || "JOURNAL"} #{draft.referenceId || draft.id}</TableCell>
                        <TableCell className="text-right font-bold">{debit.toLocaleString(undefined, { minimumFractionDigits: 2 })}</TableCell>
                        <TableCell className="text-right font-bold">{credit.toLocaleString(undefined, { minimumFractionDigits: 2 })}</TableCell>
                        <TableCell className="text-right pr-6">
                          <Button size="sm" onClick={() => postDraftMutation.mutate(draft.id)} disabled={postDraftMutation.isPending || Math.abs(debit - credit) > 0.005}>
                            Post Draft
                          </Button>
                        </TableCell>
                      </TableRow>
              ...
              <Card key={entry.id} className="border-slate-200/60 shadow-sm overflow-hidden rounded-2xl group hover:shadow-md transition-all duration-300">
                ...
                <CardContent className="p-0">
                  <Table>
                    ...
                    <TableBody>
                      {(entry.lines || entry.ledgerEntries || []).map((line) => (
