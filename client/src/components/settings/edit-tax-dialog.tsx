...
export function EditTaxDialog({ product }: { product: any }) {
  ...
  const updateProduct = useMutation({
    mutationFn: async (data: UpdateFormValues) => {
      ...
      let taxCategoryId = data.taxCategoryId
        ? parseInt(data.taxCategoryId)
        : null;

      if (taxCategoryId && taxCategories.data && taxTypes.data) {
        const category = taxCategories.data.find(
          (c: any) => c.id === taxCategoryId,
        );
        if (category && category.defaultTaxTypeId) {
          const type = taxTypes.data.find(
            (t: any) => t.id === category.defaultTaxTypeId,
          );
          if (type) {
            taxRate = type.rate;
          }
