...
try {
  Font.register({
    ...
    fonts: [
      {
        src: "https://fonts.gstatic.com/s/roboto/v30/KFOlCnqEu92Fr1MmSU5fBBc4.ttf",
        fontWeight: 300,
      },
      {
      ...
      <Page size="A4" style={[styles.page, { fontFamily: pageFont }]}>
        ...
        <View style={styles.periodBlock}>
          ...
          <Text style={styles.periodValue}>
            {cur} {Number(data.openingBalance).toFixed(2)}
          </Text>
        </View>
        ...
        <View style={styles.table}>
          ...
          {data.transactions.map((tx, i) => (
            <View key={i} style={styles.tableRow}>
              ...
              <Text style={[styles.colRef, styles.cellText]}>
                {tx.reference}
              </Text>
              <Text style={[styles.colDesc, styles.cellText]}>
                {tx.description}
              </Text>
              <Text style={[styles.colMoney, styles.cellText]}>
                {tx.debit > 0 ? tx.debit.toFixed(2) : "-"}
              </Text>
              <Text style={[styles.colMoney, styles.cellText]}>
                {tx.credit > 0 ? tx.credit.toFixed(2) : "-"}
              </Text>
              <Text style={[styles.colBal, styles.cellText]}>
        ...
        <View style={styles.balanceRow}>
          ...
          <Text style={styles.totalValue}>
            {cur} {Number(data.closingBalance).toFixed(2)}
          </Text>
        </View>
      </Page>
    </Document>
