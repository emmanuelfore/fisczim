...
async function run() {
  ...
  try {
    ...
    console.log("Zimra taxes:", zimraTaxes);

    await db.transaction(async (tx) => {
      // 1. Get current taxes
      const current = await tx.select().from(taxTypes).where(eq(taxTypes.companyId, companyId));
      ...
      for (const zTax of zimraTaxes) {
        ...
        // delete if code exists to avoid duplicate
        await tx.delete(taxTypes).where(eq(taxTypes.code, code)).where(eq(taxTypes.companyId, companyId)).catch(() => {});
      ...
      for (const old of current) {
        ...
        if (replacement) {
          ...
          await tx.update(products).set({ taxTypeId: replacement.id }).where(eq(products.taxTypeId, old.id));
          await tx.update(invoiceItems).set({ taxTypeId: replacement.id }).where(eq(invoiceItems.taxTypeId, old.id));
        } else {
      ...
      // 4. Delete old taxes
      const oldIds = current.map(t => t.id);
      if (oldIds.length > 0) {
         console.log(`Deleting old taxes: ${oldIds.join(', ')}`);
         await tx.delete(taxTypes).where(inArray(taxTypes.id, oldIds));
      }
    });
