      ...
      <CardHeader className="bg-slate-50 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between p-6">
        <div>
          ...
          <p className="text-slate-500 font-medium  mt-1">
            For the period {format(dateRange.from, "MMM d")} to{" "}
      ...
      <CardContent className="p-8">
        <div className="max-w-4xl mx-auto space-y-8">
          <div>
            ...
            <Table>
              <TableBody>
                {Object.entries(groupedInflows).map(
                  ([desc, amount]: [string, any]) => (
                    ...
                    >
                      ...
                      <TableCell className="text-right font-semibold text-emerald-600">
                        {consolidatedSymbol}
                        {(amount * consolidatedRate).toLocaleString(undefined, {
                ...
                <TableRow className="bg-slate-50/30">
                  <TableCell className="font-bold text-slate-800 text-right uppercase text-xs tracking-wider">
                    Total Inflows
                  </TableCell>
          ...
          <div>
            ...
            <Table>
              <TableBody>
                {Object.entries(groupedOutflows).map(
                  ([desc, amount]: [string, any]) => (
                    <TableRow
                      key={desc}
                      className="border-slate-50 hover:bg-slate-50/50"
          ...
          <div
            className={`p-6 rounded-2xl border ${netCashFlow >= 0 ? "bg-emerald-50 border-emerald-100 text-emerald-900" : "bg-rose-50 border-rose-100 text-rose-900"} flex justify-between items-center`}
          >
            ...
            <span className="text-2xl font-black">
              {netCashFlow < 0 ? "(" : ""}
              {consolidatedSymbol}
              {(Math.abs(netCashFlow) * consolidatedRate).toLocaleString(
                undefined,
