...
export function SecuritySettings({ company }: SecuritySettingsProps) {
  ...
  const { mutate: generateApiKey, isPending: isGeneratingKey } = useMutation({
    mutationFn: async () => {
      const res = await apiFetch(`/api/companies/${company.id}/api-key`, {
        method: "POST",
      });
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className=" text-muted-foreground">
          Manage API credentials and system audit logs
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <Card className="card-depth border-none">
          <CardHeader>
            ...
            <CardDescription>
              Secret keys for external integrations
            </CardDescription>
          </CardHeader>
        ...
        <Card className="card-depth border-none bg-slate-900 text-white overflow-hidden relative group">
          ...
          <CardContent className="space-y-6 pt-2">
            <p className=" text-slate-400 leading-relaxed font-medium">
              Maintain full transparency with a detailed history of every change
              made to your organization, products, and sales.
            </p>
            <Button
              variant="secondary"
