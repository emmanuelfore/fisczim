...
async function run() {
    try {
        console.log(`Updating ${targetNames.length} specific items to zero-rated for Rabyon (Company 91)...`);

        const lowerCaseNames = targetNames.map(n => n.toLowerCase());

        // We fetch them first to do case-insensitive match just in case
        ...
        const matchedProductIds = allProducts
            .filter(p => lowerCaseNames.includes(p.name.toLowerCase()))
            .map(p => p.id);
        ...
        if (matchedProductIds.length > 0) {
            await db.update(products)
                .set({
                    taxRate: "0.00",
                    taxTypeId: 175 // Assumes 175 is the Zero Rated tax type ID in DB
                })
                .where(inArray(products.id, matchedProductIds));
