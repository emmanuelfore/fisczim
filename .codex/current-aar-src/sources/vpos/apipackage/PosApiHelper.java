...
/* JADX INFO: loaded from: classes.jar:vpos/apipackage/PosApiHelper.class */
public class PosApiHelper {
    ...
    private static final String AAR_VERSION = "2.4.7";
    private static final String MCU_NODE_FILE = "/sys/devices/platform/mcu_power/mcu_power_pwren";
    private int ret;
    ...
    public static PosApiHelper getInstance() {
        ...
        Log.d("vpos", "vpos PosApiHelper getInstance--------------------------------------------------------00>> ");
        byte[] baud = {51, 51, 51, 51};
        synchronized (mLock) {
            if (mInstance == null) {
                mInstance = new PosApiHelper();
            }
            Log.d("vpos", "vpos PosApiHelper getInstance--------------------------------------------------------11>> ");
            PaySys.LibAdapterUartBaud(baud);
            Log.d("vpos", "vpos PosApiHelper getInstance--------------------------------------------------------22>> ");
    ...
    public int IccApduCmd(byte slot, byte[] pbInApdu, short usInApduLen, byte[] pbOut, byte[] pbOutLen) {
        return Icc.Lib_IccApduCmd(slot, pbInApdu, usInApduLen, pbOut, pbOutLen);
    }
    ...
    public int MifareReprobe(byte[] CardType, byte[] UID, byte[] ucUIDLen, byte[] SAK) {
        return Picc.Lib_MifareReprobe(CardType, UID, ucUIDLen, SAK);
    }

    public int PiccPollingP3(byte[] CardType, byte[] UID, byte[] ucUIDLen, byte[] SAK) {
        return Picc.Lib_PiccPollingP3(CardType, UID, ucUIDLen, SAK);
    }

    public int PiccM1Authority(byte type, int blkNo, byte[] pwd, byte[] serialNo) {
    ...
    public int PiccM1Operate(byte type, int blkNo, byte[] value, byte updateBlkNo) {
    ...
    public int PrintCheckStatus() {
        ...
        int ret = Print.Lib_PrnSetVoltage((voltage * 2) / 100000);
        if (ret != 0) {
            return ret;
        }
    ...
    public int SetCurrentBaudRate(byte BaudRate) {
        return Sys.Lib_SetCurrentBaudRate(BaudRate);
    }

    public int GetCurrentBaudRate() {
        int ret = Sys.Lib_GetCurrentBaudRate();
        Log.e("vpos", "Lib_GetCurrentBaudRate ret" + ret);
        return ret;
    }

    public int SysLogSwitch(byte LogSwitch) {
        PaySys.LogTurnOn(LogSwitch);
        Log.e("vpos", "SysLogSwitch AAR");
    ...
    public int ReadRegister(byte[] Address, byte[] Register) {
        return Sys.Lib_ReadRegister(Address, Register);
    }

    public int WriteRegister(byte[] Sn) {
        return Sys.Lib_WriteRegister(Sn);
    }

    public int ReadEEprom(byte[] Address, byte[] EEprom) {
        return Sys.Lib_ReadEEprom(Address, EEprom);
    }

    public int WriteEEprom(byte[] Sn) {
        return Sys.Lib_WriteEEprom(Sn);
    }

    public int LoadRfConfig(byte[] Sn) {
        return Sys.Lib_LoadRfConfig(Sn);
    }

    public int UpdateRfConfig(byte[] Sn) {
        return Sys.Lib_UpdateRfConfig(Sn);
    }

    public int RetrieveRfConfiguration(byte[] jSn, byte[] jppRfConfiguration) {
        return Sys.Lib_RetrieveRfConfiguration(jSn, jppRfConfiguration);
    }

    public int RetrieveRfConfigurationSize(byte[] jSn, byte[] jRlen) {
        return Sys.Lib_RetrieveRfConfigurationSize(jSn, jRlen);
    }

    public int ReadAutoLoadRf(byte[] Status) {
        return Sys.Lib_ReadAutoLoadRf(Status);
    }

    public int CheckRfConfigStatus(byte[] Status) {
        return Sys.Lib_CheckRfConfigStatus(Status);
    }

    public int ReadAutoSetRf(byte[] Status) {
        return Sys.Lib_ReadAutoSetRf(Status);
    }

    public int SetAutoLoadRf(byte Status) {
        return Sys.Lib_SetAutoLoadRf(Status);
    }

    public int SetAutoSetRf(byte Status) {
        return Sys.Lib_SetAutoSetRf(Status);
    }
