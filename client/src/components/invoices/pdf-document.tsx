...
export const InvoicePDF = ({
  ...
  const bankingDetails = [
    ...
    seller?.accountName
      ? `Acc Name: ${seller.accountName || seller.name}`
      : null,
  ...
  const hasBankingDetails =
    bankingDetails.length > 0 || Boolean(seller?.bankDetails);

  const designerSettings =
    templateSettings ||
    getStoredInvoiceTemplateSettings(seller?.id || invoice?.companyId);
  const template = getInvoiceTemplate(
    designerSettings.defaultTemplateId ||
      seller?.invoiceTemplate ||
      invoice.invoiceTemplate,
  );
  const accentColor =
    designerSettings.accentColor ||
    seller?.primaryColor ||
    template.accent ||
    "#2563eb";
  const compact =
    designerSettings.density === "compact" || template.density === "compact";
  ...
  const verificationCodeRaw = invoice.qrCodeData
    ? invoice.qrCodeData.slice(-16)
    : "";
  const verificationCode =
    verificationCodeRaw.match(/.{1,4}/g)?.join("-") || "";
      ...
      >
        ...
        <View
          style={{
            ...
            padding: template.headerMode === "band" ? 10 : 0,
            paddingBottom: template.headerMode === "band" ? 10 : 10,
            borderBottomWidth: 1,
            borderBottomColor: borderColor,
            backgroundColor:
              template.headerMode === "band"
                ? template.secondary
                : "transparent",
          }}
        >
          ...
          <View style={{ width: "28%" }}>
            {seller?.logoUrl ? (
              <Image
                src={seller.logoUrl}
                style={{ width: 108, height: 50, objectFit: "contain" }}
              />
            ) : null}
          </View>
          ...
          <View style={{ width: "44%", alignItems: "center" }}>
            {showHeaderQr &&
            designerSettings.qrPlacement === "header-center" ? (
              <Image
                style={{ width: 62, height: 62, marginBottom: 3 }}
                src={qrCodeUrl}
              />
            ) : null}
            {invoice.fiscalCode && invoice.status !== "quote" ? (
              <>
                ...
                <Text
                  style={{ fontSize: 7, color: "blue", textDecoration: "none" }}
                >
            ...
            {showHeaderQr && designerSettings.qrPlacement === "header-right" ? (
              ...
              <View style={{ alignItems: "flex-end" }}>
                ...
                {invoice.expiryDate ? (
                  <Text style={{ fontSize: 8, color: "#64748b" }}>
                    Valid Until:{" "}
                    {format(new Date(invoice.expiryDate), "dd/MM/yyyy")}
                  </Text>
                ) : null}
              </View>
            ) : null}
          ...
          >
            <View style={{ width: "48%" }}>
              <View style={styles.fiscalRow}>
                <Text style={styles.fiscalLabel}>
                  {invoice.status === "quote" ? "Quote No:" : "Invoice No:"}
                </Text>
                <Text style={styles.fiscalValue}>
                  {invoice.status !== "quote" &&
                  invoice.receiptCounter !== null &&
                  invoice.receiptCounter !== undefined &&
                  invoice.receiptGlobalNo !== null &&
                  invoice.receiptGlobalNo !== undefined
              ...
              {invoice.status !== "quote" && (
                <>
                  <View style={styles.fiscalRow}>
                    ...
                    <Text style={styles.fiscalValue}>
                      {seller?.fdmsDeviceId || "N/A"}
                    </Text>
                  </View>
                  <View style={styles.fiscalRow}>
                    <Text style={styles.fiscalLabel}>Device Serial No:</Text>
                    <Text style={styles.fiscalValue}>
                      {seller?.fdmsDeviceSerialNo || "N/A"}
                    </Text>
                  </View>
                </>
              )}
        ...
        {(invoice.transactionType === "CreditNote" ||
          invoice.transactionType === "DebitNote") &&
        Boolean(invoice.relatedInvoiceId) ? (
          <View
            style={{
              marginTop: 6,
              padding: 5,
          ...
          >
            ...
            >
              ...
              <Text style={{ fontSize: 7, color: "#64748b", marginLeft: 8 }}>
            ...
            <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 10 }}>
              <Text style={{ fontSize: 7 }}>
                ...
                <Text style={{ fontWeight: 700 }}>
                  {invoice.relatedReceiptGlobalNo || "N/A"}
                </Text>
              </Text>
              <Text style={{ fontSize: 7 }}>
                <Text style={{ color: "#64748b" }}>Date: </Text>
                <Text style={{ fontWeight: 700 }}>
                  {invoice.relatedInvoiceDate
                    ? format(
                        new Date(invoice.relatedInvoiceDate),
                        "dd/MM/yyyy HH:mm",
                      )
                    : "N/A"}
                </Text>
              ...
              <Text style={{ fontSize: 7 }}>
                ...
                <Text style={{ fontWeight: 700 }}>
                  {invoice.relatedInvoiceNumber || "N/A"}
                </Text>
              </Text>
              <Text style={{ fontSize: 7 }}>
        ...
        <View style={styles.table}>
          <View
            style={[
              styles.tableHeader,
              { backgroundColor: accentColor, borderRadius: template.radius },
            ]}
          >
            {seller?.vatRegistered ? (
              isExclusive ? (
                <>
                  <Text style={[styles.colExCode, styles.headerText]}>
                    Code
                  </Text>
                  <Text style={[styles.colExDesc, styles.headerText]}>
                    Description
                  </Text>
                  <Text style={[styles.colExQty, styles.headerText]}>Qty</Text>
                  <Text style={[styles.colExPrice, styles.headerText]}>
                    Price
                  </Text>
                  <Text style={[styles.colExAmt, styles.headerText]}>
                    Amount{"\n"}(excl.)
                  </Text>
                  <Text style={[styles.colExVat, styles.headerText]}>VAT</Text>
                  <Text style={[styles.colExTotal, styles.headerText]}>
                    Total{"\n"}(incl.)
                  </Text>
                </>
              ) : (
              ...
              <>
                <Text
                  style={[styles.colCode, styles.headerText, { width: "15%" }]}
                >
                  Code
                </Text>
                <Text
                  style={[styles.colDesc, styles.headerText, { width: "45%" }]}
                >
                  Description
                </Text>
                <Text
                  style={[styles.colQty, styles.headerText, { width: "10%" }]}
                >
                  Qty
                </Text>
                <Text
                  style={[styles.colPrice, styles.headerText, { width: "15%" }]}
                >
                  Price
                </Text>
                <Text
                  style={[styles.colTotal, styles.headerText, { width: "15%" }]}
                >
                  Total
                </Text>
              </>
          ...
          {invoice.items?.map((item: any, i: number) => {
            ...
            const effectiveTaxRate = seller?.vatRegistered
            ...
            if (isExclusive) {
              ...
              vatAmt =
                displayTotalIncl - displayTotalIncl / (1 + taxRate / 100);
            }
            ...
            const isExempt =
              matchingTax?.zimraTaxId == 1 ||
              matchingTax?.zimraTaxId == "1" ||
              matchingTax?.name?.toLowerCase().includes("exempt");
            const isZeroRated =
              matchingTax?.zimraTaxId == 2 ||
              matchingTax?.zimraTaxId == "2" ||
              matchingTax?.name?.toLowerCase().includes("zero rated") ||
              (!isExempt && taxRate === 0);
            ...
            return (
              <View
                ...
                style={[
                  styles.tableRow,
                  { borderBottomColor: borderColor },
                  i % 2 === 1 ? { backgroundColor: sectionBg } : {},
                ]}
              >
                {seller?.vatRegistered ? (
                  isExclusive ? (
                    <>
                      <Text style={styles.colExCode}>
                        {item.product?.hsCode || "0000"}
                      </Text>
                      ...
                      <Text style={styles.colExPrice}>
                        {displayPrice.toFixed(2)}
                      </Text>
                      <Text style={styles.colExAmt}>
                        {displayAmtExcl.toFixed(2)}
                      </Text>
                      <Text style={styles.colExVat}>
                        {isExempt
                          ? "-"
                          : isZeroRated || vatAmt === 0
                            ? "0.00"
                            : vatAmt.toFixed(2)}
                      </Text>
                      <Text style={styles.colExTotal}>
                        {displayTotalIncl.toFixed(2)}
                      </Text>
                    </>
                  ) : (
                    <>
                  ...
                  <>
                    <Text style={[styles.colCode, { width: "15%" }]}>
                      {item.product?.hsCode || "0000"}
                    </Text>
                    <Text style={[styles.colDesc, { width: "45%" }]}>
                      {item.description}
                    </Text>
                    <Text style={[styles.colQty, { width: "10%" }]}>{qty}</Text>
                    <Text style={[styles.colPrice, { width: "15%" }]}>
                      {displayPrice.toFixed(2)}
                    </Text>
                    <Text style={[styles.colTotal, { width: "15%" }]}>
                      {displayTotalIncl.toFixed(2)}
                    </Text>
                  </>
                )}
        ...
        <View style={styles.summarySection}>
          ...
          {isTaxPayer ? (
            <View
              style={[
                styles.taxTable,
                { borderColor, borderRadius: template.radius },
              ]}
            >
              <Text
                style={[
                  styles.sectionTitle,
                  { marginBottom: 6, color: accentColor },
                ]}
              >
                Tax Analysis
              </Text>
              ...
              >
                <Text style={{ fontSize: 8, width: "25%", color: "#64748b" }}>
                  VAT %
                </Text>
                <Text
                  style={{
              ...
              {(() => {
                ...
                const taxSummary =
                  invoice.items?.reduce(
                    (acc: any, item: any) => {
                      const taxRate = seller?.vatRegistered
                        ? Number(item.taxRate || item.product?.taxRate || 0)
                        : 0;
                ...
                return Object.entries(taxSummary).map(
                  ([key, data]: [string, any]) => (
                    <View
                      key={key}
                    ...
                    >
                      <Text style={{ fontSize: 8, width: "25%" }}>
                        {(() => {
                          const mTax = taxTypes?.find(
                            (t) => t.id == data.taxTypeId,
                          );
                          const isExempt =
                            mTax?.zimraTaxId == 1 ||
                            mTax?.zimraTaxId == "1" ||
                      ...
                      >
                        {(() => {
                          ...
                          const isZeroRated =
                            ...
                            (!isExempt && data.taxRate === 0);

                          return isExempt
                            ? "-"
                            : isZeroRated || data.taxAmount === 0
                              ? "0.00"
          ...
          >
            ...
            <View style={[styles.grandTotal, { borderTopColor: accentColor }]}>
              <Text>
                Total amount {seller?.vatRegistered ? "(incl. tax)" : ""}
              </Text>
              <Text>
            ...
            {bankingDetails.length > 0 ? (
              ...
              >
                {bankingDetails.map((detail, index) => (
                  <Text
                    key={detail}
                    style={{ fontSize: 8, color: "#475569", marginRight: 8 }}
                  >
                    {index > 0 ? (
        ...
        {showFooterQr ? (
          ...
          >
            ...
            <View>
              ...
              >
                ZIMRA Verification QR
              </Text>
              <Text style={{ fontSize: 7, color: "#64748b" }}>
                Scan to verify this fiscal document.
              </Text>
              {verificationCode ? (
        ...
        <View style={styles.footerSection}>
          ...
          <Text
            style={{
              fontSize: 7,
              color: invoice.fiscalCode ? "#94a3b8" : "#b45309",
              textAlign: "center",
            }}
          >
            {invoice.fiscalCode
