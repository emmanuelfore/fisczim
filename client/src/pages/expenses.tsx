...
export default function ExpensesPage() {
  ...
  const totalPages = Math.ceil(
    (filteredExpenses?.length || 0) / ITEMS_PER_PAGE,
  );
  ...
  const handleExportExcel = () => {
    downloadExcel(
      `/api/reports/export/expenses/${companyId}`,
      `Expenses_Report_${format(new Date(), "yyyyMMdd")}.xlsx`,
    );
  };
          ...
          <div className="hidden md:block overflow-x-auto min-w-0">
            ...
            <thead>
              <tr className="bg-slate-50/50 border-b border-slate-100 text-[10px] uppercase tracking-widest font-bold text-slate-500">
                ...
                <th className="hidden sm:table-cell p-6 font-bold text-slate-400">
                  Category
                </th>
                <th className="hidden lg:table-cell p-6 font-bold text-slate-400">
                  Vendor
                </th>
                <th className="hidden md:table-cell p-6 font-bold text-slate-400">
                  Reference
                </th>
