  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiFetch(
        buildUrl(companyId, "sales-summary", dateRange),
      );
      if (!res.ok)
        throw new Error(`Failed to load sales summary (${res.status})`);
      return res.json();
    },
    enabled: !!companyId,
  });
        ...
        {!selectedRow ? (
          ...
          <div className="space-y-3">
            <h3 className=" font-black text-slate-800 uppercase tracking-tight">
              {isValid(new Date(selectedRow.date))
...
export function SalesByCustomerReport({
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiFetch(
        buildUrl(companyId, "sales-by-customer", dateRange),
      );
      if (!res.ok)
        throw new Error(`Failed to load sales by customer (${res.status})`);
      return res.json();
    },
    enabled: !!companyId,
  });
...
export function SalesByItemReport({
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiFetch(
        buildUrl(companyId, "sales-by-item", dateRange),
      );
      if (!res.ok)
        throw new Error(`Failed to load sales by item (${res.status})`);
      return res.json();
    },
    enabled: !!companyId,
  });
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                <div key={label} className="bg-slate-50 rounded-xl p-3">
                  ...
                  <p className=" font-black text-slate-800">{String(value)}</p>
                </div>
              ))}
            </div>
          </div>
...
export function SalesBySalespersonReport({
  ...
  const {
    ...
    queryFn: async () => {
      const res = await apiFetch(
        buildUrl(companyId, "sales-by-salesperson", dateRange),
      );
      if (!res.ok)
        throw new Error(`Failed to load sales by salesperson (${res.status})`);
      return res.json();
    },
    enabled: !!companyId,
  });
