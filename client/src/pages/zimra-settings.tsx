...
export default function ZimraSettingsPage() {
  ...
  if (!currentCompany)
    return (
      <Layout>
        <div className="p-8">No company details available.</div>
      </Layout>
    );
...
function ZimraDeviceConfig({ company }: { company: any }) {
  ...
  const renewCertificateMutation = useMutation({
    mutationFn: async () => {
      const res = await apiFetch(
        `/api/companies/${company.id}/zimra/issue-certificate`,
        {
          method: "POST",
        },
      );
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/companies"] });
      toast({
        title: "Certificate Renewed Successfully!",
        className: "bg-green-100 text-green-900",
      });
    },
    onError: (err: any) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-slate-100 p-1 rounded-lg flex items-center w-fit mb-6">
        ...
        <Button
          variant={
            company.zimraEnvironment === "production" ? "destructive" : "ghost"
          }
          size="sm"
          ...
          onClick={() => {
            ...
            if (
              !hasActiveSubForThisMachine &&
              company.subscriptionStatus !== "active"
            ) {
              toast({
                title: "Subscription Required",
                description:
                  "An active subscription is required to switch to Production mode.",
                variant: "destructive",
              });
              return;
            }
      ...
      <div className="flex items-center justify-between mb-6 p-3 bg-slate-50 border rounded-lg">
        <div className="flex items-center gap-3">
          <div
            className={`p-2 rounded-full ${company.subscriptionStatus === "active" ? "bg-green-100" : "bg-slate-100"}`}
          >
            <Zap
              className={`w-4 h-4 ${company.subscriptionStatus === "active" ? "text-green-600" : "text-slate-400"}`}
            />
          </div>
          <div>
            ...
            <div className="flex items-center gap-2">
              {company.subscriptionStatus === "active" ? (
                ...
                <Badge
                  variant="outline"
                  className="text-[10px] h-4 bg-slate-50 text-slate-500"
                >
                  INACTIVE
                </Badge>
              )}
        ...
        <Link href="/subscription">
          <div>
            ...
            <p className="text-[11px] text-slate-500">
              Enable production access ($150.00 / year)
            </p>
          </div>
      ...
      {verificationResult && (
        <div className="bg-slate-50 p-4 rounded-md border  space-y-1">
          <p>
            <strong>TaxPayer:</strong> {verificationResult.taxPayerName}
          </p>
          <p>
            <strong>TIN:</strong> {verificationResult.taxPayerTIN}
          </p>
          <p>
            <strong>Address:</strong>{" "}
            {verificationResult.deviceBranchAddress?.city},{" "}
            {verificationResult.deviceBranchAddress?.street}
          </p>
        </div>
      )}
        ...
        {!isRegistered || isEditing ? (
          <>
            ...
            <Button
              ...
              disabled={
                registerDeviceMutation.isPending || !deviceId || !activationKey
              }
            >
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className="text-xs opacity-75">
                  ...
                  <span className="font-mono font-bold text-blue-600">
                    {company.fdmsDeviceSerialNo || "Pending"}
                  </span>
                </span>
                <span className="text-[10px] opacity-50">|</span>
                <span className="text-xs opacity-75">
                  {company.fiscalDayOpen
                    ? `Day ${company.currentFiscalDayNo || "?"}`
                    : "No Active Day"}
                </span>
              </div>
            </div>
          ...
          {connectivityResult && (
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border">
                ...
                <Badge
                  variant={
                    connectivityResult.overallStatus === "Online"
                      ? "default"
                      : "destructive"
                  }
                  className={
              ...
              <div className="text-xs text-slate-400 text-center pt-2">
                Last Tested:{" "}
                {new Date(connectivityResult.timestamp).toLocaleTimeString()}
              </div>
            </div>
          )}
        </DialogContent>
      ...
      {isRegistered && (
        <div className="pt-6 border-t space-y-4">
          <div>
            <h4 className=" font-semibold mb-2 text-slate-700 flex items-center gap-2">
          ...
          <div>
            <h4 className=" font-semibold mb-2 text-slate-700">Maintenance</h4>
            <div className="flex gap-2 flex-wrap">
              <Button
                variant="outline"
                size="sm"
                onClick={() => renewCertificateMutation.mutate()}
              >
                <RefreshCw className="w-4 h-4 mr-2" />
            ...
            <AccordionItem value="danger_zone" className="border-none">
              ...
              <AccordionContent className="p-4 bg-slate-50 space-y-4">
                <p className="text-xs text-slate-500">
                  Manually reset critical device counters.{" "}
                  <strong>Use with extreme caution</strong> - only when
                  switching devices or fixing synchronization errors.
                </p>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
      ...
      <div className="md:col-span-3 space-y-2">
        ...
        <p className="text-[10px] text-slate-500">
          Only change this if you know the exact hash of the last successfully
          submitted receipt.
        </p>
      </div>
    </div>
