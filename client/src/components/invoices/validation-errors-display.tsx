    ...
    >
      ...
      <CardContent className="space-y-4">
        ...
        >
          ...
          <AlertDescription>
            {hasRedErrors &&
              "This receipt contains major validation errors (RED) that prevent fiscal day closure."}
            {hasGreyErrors &&
              !hasRedErrors &&
              "This receipt has missing previous receipts in the chain (GREY)."}
            {!hasRedErrors &&
              !hasGreyErrors &&
              "This receipt contains minor validation errors (YELLOW)."}
          </AlertDescription>
        </Alert>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <Badge
                    variant={getErrorBadgeVariant(error.errorColor) as any}
                  >
                    {error.errorCode}
                  </Badge>
                ...
                {error.requiresPreviousReceipt && (
                  <p className="text-xs text-gray-600 mt-1">
                    This error requires the previous receipt to be present in
                    the chain.
                  </p>
                )}
              </div>
        ...
        {(onResubmit || onEdit) && (
          <div className="flex gap-2 pt-2 border-t">
            {onResubmit && (
              ...
              >
                <RefreshCw
                  className={`w-4 h-4 ${isResubmitting ? "animate-spin" : ""}`}
                />
