...
import { createCostLayer, consumeInventory, recalculateBranchAVCO } from "./costing";
...
export async function calculateCOGS(
    ...
    const method = (company?.method || "WAC") as 'FIFO' | 'LIFO' | 'AVCO';
...
export async function recordStockIn(
    ...
    const components = [
        { type: 'BASE' as const, unitCost, totalCost: quantity * unitCost }
    ];
...
export async function recordBatchStockIn(
    ...
    await db.transaction(async (tx) => {
        for (const item of allocatedItems) {
            ...
            // 2. Create Cost Layers
            const components = [
                { type: 'BASE' as const, unitCost: baseUnitCost, totalCost: quantity * baseUnitCost }
            ];
...
export async function recordTransfer(
    ...
    for (const item of items) {
        ...
        const components = [
            { type: 'BASE' as const, unitCost: unitCost, totalCost: totalCogs }
        ];
