...
export default function InventoryReportsPage() {
  ...
  const valuationMethod =
    products?.find((p: any) => p.valuationMethod)?.valuationMethod || "WAC";
  ...
  // Calculations
  const lowStockItems =
    products?.filter((p: any) => Number(p.stockLevel) <= 5) || [];
  const outOfStockItems =
    products?.filter((p: any) => Number(p.stockLevel) <= 0) || [];

  const totalCostValue =
    products?.reduce((acc: number, p: any) => acc + p.totalValuation, 0) || 0;
  ...
  const handleExportCsv = () => {
    ...
    link.setAttribute(
      "download",
      `Inventory_Valuation_${format(new Date(), "yyyyMMdd")}.csv`,
    );
  ...
  const handleExportAdjustmentsCsv = () => {
    ...
    link.setAttribute(
      "download",
      `Stock_Adjustments_${format(new Date(), "yyyyMMdd")}.csv`,
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const filteredAdjustments =
    adjustments?.filter((a: any) => {
      const matchesSearch =
        a.productName.toLowerCase().includes(adjustmentSearch.toLowerCase()) ||
        (a.sku || "").toLowerCase().includes(adjustmentSearch.toLowerCase()) ||
        (a.notes || "")
          .toLowerCase()
          .includes(adjustmentSearch.toLowerCase()) ||
        (a.userName || "")
          .toLowerCase()
          .includes(adjustmentSearch.toLowerCase());

      const matchesType =
        adjustmentTypeFilter === "all" || a.type === adjustmentTypeFilter;
      return matchesSearch && matchesType;
    }) || [];

  const adjustmentEvents = filteredAdjustments.length;
  const netAdjustmentQty = filteredAdjustments.reduce(
    (sum: number, a: any) => sum + Number(a.quantity || 0),
    0,
  );
  const negativeAdjustments = filteredAdjustments.filter(
    (a: any) => Number(a.quantity || 0) < 0,
  ).length;
  const uniqueAdjusters = new Set(
    filteredAdjustments.map((a: any) => a.userName || "System"),
  ).size;
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            variant="outline"
            className="gap-2 rounded-2xl border-slate-200 shadow-sm hover:bg-slate-50"
          >
          ...
          <Card className="lg:col-span-1 border-none shadow-xl bg-white rounded-[2rem] overflow-hidden">
            <CardHeader className="p-8 pb-0">
              ...
              <CardDescription className="text-slate-400 font-medium">
                Highest value products in stock
              </CardDescription>
            </CardHeader>
              ...
              <Table>
                ...
                <TableBody>
                  {lowStockItems.slice(0, 5).map((p: any) => (
                    <TableRow
                      key={p.productId}
                      className="hover:bg-slate-50/80 border-b border-slate-50 transition-colors duration-200"
                    >
                      ...
                      <TableCell className="p-6 text-right font-black text-amber-600">
                        {p.stockLevel}
                      </TableCell>
                      <TableCell className="p-6 text-right text-slate-500 font-medium ">
        ...
        <Card className="border-none shadow-xl bg-white rounded-[2rem] overflow-hidden">
          <CardHeader className="p-8 border-b border-slate-50 flex flex-row items-center justify-between bg-slate-50/30">
            <div>
              ...
              <CardDescription className="text-slate-400 font-medium">
                Detailed cost-based valuation list using {valuationMethod}
              </CardDescription>
            </div>
                  ...
                  filteredProducts?.map((p: any) => (
                    ...
                    >
                      <TableCell className="p-6">
                        ...
                        <div className="text-[10px] text-slate-400 font-mono tracking-tighter uppercase mt-1">
                          {p.sku}
                        </div>
                      </TableCell>
                      ...
                      <TableCell className="p-6 text-right text-slate-500 font-mono text-xs">
                        {p.valuationMethod || valuationMethod}
                      </TableCell>
                      <TableCell className="p-6 text-right font-black text-slate-900 text-lg font-display">
                        $
                        {p.totalValuation.toLocaleString(undefined, {
                          minimumFractionDigits: 2,
                        })}
                      </TableCell>
                      <TableCell className="p-6">
                        {Number(p.stockLevel) <= 0 ? (
                          <span className="px-3 py-1 bg-red-50 text-red-600 text-[10px] font-black uppercase rounded-full tracking-tighter border border-red-100">
                            Out of Stock
                          </span>
                        ) : Number(p.stockLevel) <= 5 ? (
                          <span className="px-3 py-1 bg-amber-50 text-amber-600 text-[10px] font-black uppercase rounded-full tracking-tighter border border-amber-100">
                            Low Stock
                          </span>
                        ) : (
                          <span className="px-3 py-1 bg-emerald-50 text-emerald-600 text-[10px] font-black uppercase rounded-full tracking-tighter border border-emerald-100">
                            Healthy
                          </span>
                        )}
                      </TableCell>
        ...
        <Card className="border-none shadow-xl bg-white rounded-[2rem] overflow-hidden">
          <CardHeader className="p-8 border-b border-slate-50 flex flex-row items-center justify-between bg-slate-50/30">
            <div>
              ...
              <CardDescription className="text-slate-400 font-medium">
                Track all manual stock changes, reasons, and users
              </CardDescription>
            </div>
            <Button
          ...
          <CardContent className="p-8 space-y-6">
            ...
            <div className="flex flex-col lg:flex-row gap-3">
              ...
              <select
                className="h-11 rounded-2xl border border-slate-200 bg-white px-3  font-semibold text-slate-700"
              ...
              <Input
                ...
                className="h-11 rounded-2xl border-slate-200 w-full lg:w-[180px]"
              />
              <Input
                type="date"
            ...
            <div className="rounded-2xl border border-slate-100 overflow-hidden">
              <Table>
                <TableHeader className="bg-slate-50/80 hover:bg-slate-50/80 border-b border-slate-100">
                  <TableRow>
                    <TableHead className="p-5 font-bold text-slate-400 uppercase tracking-widest text-[10px]">
                      Date
                    </TableHead>
                    <TableHead className="p-5 font-bold text-slate-400 uppercase tracking-widest text-[10px]">
                      Product
                    ...
                    <TableHead className="p-5 font-bold text-slate-400 uppercase tracking-widest text-[10px] text-right">
                      Qty Change
                    </TableHead>
                    <TableHead className="p-5 font-bold text-slate-400 uppercase tracking-widest text-[10px]">
                      User
                  ...
                  {isAdjustmentsLoading ? (
                    ...
                    filteredAdjustments.map((a: any) => (
                      ...
                      >
                        ...
                        <TableCell
                          className={`p-5 text-right font-black ${Number(a.quantity) < 0 ? "text-rose-600" : "text-emerald-600"}`}
                        >
                          {Number(a.quantity) > 0 ? "+" : ""}
                          {Number(a.quantity).toFixed(2)}
                        </TableCell>
                        <TableCell className="p-5 text-xs font-semibold text-slate-700">
                          {a.userName || "System"}
                        </TableCell>
                        <TableCell className="p-5 text-xs text-slate-500 font-mono">
                          {a.reference || "-"}
                        </TableCell>
                        <TableCell className="p-5 text-xs text-slate-500 max-w-[320px] truncate">
                          {a.notes || "-"}
                        </TableCell>
                      </TableRow>
                    ))
