...
export default function RecentSalesPage() {
  ...
  const totalRevenue =
    sales?.reduce((sum: number, s: any) => sum + Number(s.total), 0) || 0;
  const totalTxns = sales?.length || 0;
  const fiscalizedCount =
  ...
  const handleExportCsv = () => {
    ...
    link.setAttribute(
      "download",
      `sales-report-${format(dateRange.from, "MMM-dd")}-to-${format(dateRange.to, "MMM-dd")}.csv`,
    );
          ...
          <div className="flex items-center gap-4">
            <Link href="/reports/pos">
              <Button
                variant="outline"
                size="icon"
                className="h-12 w-12 rounded-2xl border-slate-200 bg-white shadow-sm hover:bg-slate-50"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          ...
          <Card className="border-none shadow-sm bg-amber-500 text-white rounded-[2rem]">
            <CardContent className="p-6">
              <div className="flex justify-between items-start">
                <div>
                  <p className="text-amber-100 text-[10px] font-black uppercase tracking-widest mb-1">
                    Pending Sync
                  </p>
                  <h3 className="text-3xl font-black">
                    {totalTxns - fiscalizedCount}
                  </h3>
                </div>
            ...
            <div className="flex flex-wrap gap-4">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className="h-12 px-6 rounded-2xl border-slate-100 bg-slate-50/50 hover:bg-white flex items-center gap-3 font-bold text-slate-600"
                  >
              ...
              <Select value={statusFilter} onValueChange={setStatusFilter}>
                ...
                <SelectContent className="rounded-2xl border-slate-100 p-1">
                  ...
                  <SelectItem value="fiscalized" className="rounded-xl">
                    Fiscalized
                  </SelectItem>
          ...
          <Table>
            <TableHeader className="bg-slate-50/50 border-b border-slate-100">
              <TableRow className="hover:bg-transparent">
                <TableHead className="w-[160px] h-14 font-black text-[10px] uppercase tracking-widest text-slate-400 pl-8">
                  Date / Time
                </TableHead>
                ...
                <TableHead className="h-14 font-black text-[10px] uppercase tracking-widest text-slate-400 text-right">
                  Final Amount
                </TableHead>
                {user?.isSuperAdmin && (
                  ...
                  >
                    ...
                    <TableCell>
                      {s.syncedWithFdms ? (
                        ...
                        <Badge
                          variant="outline"
                          className="border-amber-200 bg-amber-50 text-amber-600 rounded-xl px-4 py-1.5 font-black text-[10px] uppercase tracking-widest flex items-center gap-2 w-fit"
                        >
                      ...
                      <TableCell className="text-right pr-8">
                        <Button
                          ...
                          onClick={async () => {
                            if (
                              confirm(
                                `Are you sure you want to delete invoice ${s.invoiceNumber}? This action cannot be undone.`,
                              )
                            ) {
                              try {
