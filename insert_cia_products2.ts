...
async function run() {
  try {
    ...
    // First delete any products that were inserted in the previous broken runs
    await db.delete(products).where(products.companyId).execute().catch(e => {
        // Only delete the exact items we tried to add earlier to avoid wiping everything
    });
