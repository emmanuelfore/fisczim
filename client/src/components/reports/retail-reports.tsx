...
export function OperationalMetricsReport({
  ...
  const { data, isLoading } = useQuery<any>({
    queryKey: [
      api.reports.operationalMetrics.path,
      companyId,
      dateRange.from,
      dateRange.to,
    ],
    ...
    <div className="p-6 space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-none shadow-md bg-slate-50">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-bold uppercase text-slate-500 flex items-center gap-2">
              <ShoppingCart className="w-4 h-4 text-blue-500" /> Avg.
              Transaction (ATV)
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-black text-slate-900">
              ${(data?.atv || 0).toFixed(2)}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-md bg-slate-50">
          ...
          <CardContent>
            <div className="text-3xl font-black text-emerald-600">
              {Math.round(data?.profitMargin || 0)}%
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-none shadow-md bg-slate-50">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-bold uppercase text-slate-500 flex items-center gap-2">
              <ShoppingBag className="w-4 h-4 text-purple-500" /> Items per
              Receipt
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="text-3xl font-black text-slate-900">
              {(data?.itemsPerReceipt || 0).toFixed(1)}
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center py-2 border-b border-slate-100">
              ...
              <span className="text-lg font-black text-rose-600">
                ${(data?.totalCogs || 0).toFixed(2)}
              </span>
            </div>
            <div className="flex justify-between items-center py-2 bg-emerald-50 px-4 rounded-xl">
              <span className=" font-bold text-emerald-700 uppercase tracking-tight">
                Gross Profit
              </span>
              <span className="text-xl font-black text-emerald-700">
                $
                {((data?.totalRevenue || 0) - (data?.totalCogs || 0)).toFixed(
                  2,
                )}
              </span>
            </div>
...
export function HourlySalesAnalysisReport({
  companyId,
  dateRange,
}: ReportProps) {
  const { data, isLoading } = useQuery<any[]>({
    queryKey: [
      api.reports.hourlySales.path,
      companyId,
      dateRange.from,
      dateRange.to,
    ],
                ...
                >
                  {data?.map((entry, index) => (
                    <Cell
                      ...
                      fill={
                        entry.total >
                        Math.max(...data.map((d) => d.total)) * 0.8
                          ? "hsl(var(--primary))"
                          : "hsl(var(--primary) / 0.4)"
                      }
                    />
      ...
      <div className="w-1/2 border-r border-slate-100 overflow-auto">
        ...
        <table className="w-full text-xs">
          <thead className="sticky top-[41px] bg-white border-b border-slate-100 z-10">
            <tr>
              <th className="text-left px-6 py-3 font-black text-slate-400 uppercase tracking-widest text-[9px]">
                Product
              </th>
              <th className="text-right px-6 py-3 font-black text-slate-400 uppercase tracking-widest text-[9px]">
                Stock
              </th>
              <th className="text-right px-6 py-3 font-black text-slate-400 uppercase tracking-widest text-[9px]">
                Value
        ...
        {!selectedRow ? (
          <div className="flex flex-col items-center justify-center h-full text-slate-300 gap-3">
            ...
            <p className=" font-medium">
              Select a product to view valuation details
            </p>
          </div>
        ) : (
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white p-4 rounded-2xl shadow-sm space-y-1">
                ...
                <p className=" font-black text-slate-800">
                  {selectedRow.category || "General"}
                </p>
              </div>
              <div className="bg-white p-4 rounded-2xl shadow-sm space-y-1">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                  Unit Cost (Avg/Current)
                </p>
                <p className=" font-black text-slate-800">
                  ${Number(selectedRow.unitCost).toFixed(2)}
                </p>
              </div>
              <div className="bg-white p-4 rounded-2xl shadow-sm space-y-1 border-l-4 border-blue-500">
                ...
                <p className="text-2xl font-black text-slate-900">
                  {Number(selectedRow.stockLevel).toFixed(0)}
                </p>
              </div>
              <div className="bg-white p-4 rounded-2xl shadow-sm space-y-1 border-l-4 border-emerald-500">
                ...
                <p className="text-2xl font-black text-emerald-700">
                  ${Number(selectedRow.totalValue).toFixed(2)}
                </p>
              </div>
            </div>
          </div>
...
export function InventoryMovementsReport({
  ...
  const { data, isLoading } = useQuery<any[]>({
    queryKey: [
      api.reports.inventoryMovements.path,
      companyId,
      dateRange.from,
      dateRange.to,
    ],
      ...
      <div className="flex-1 overflow-auto p-6 bg-slate-50/30">
        {!selectedRow ? (
          <div className="flex flex-col items-center justify-center h-full text-slate-300 gap-3">
            ...
            <p className=" font-medium">
              Select a movement to view transaction details
            </p>
          </div>
        ) : (
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-black text-slate-900 leading-tight">
                  Stock{" "}
                  {selectedRow.type === "STOCK_IN" ? "Arrival" : "Reduction"}
                </h3>
                <p className=" font-bold text-slate-500 uppercase tracking-tighter">
                  {format(new Date(selectedRow.date), "dd MMM yyyy 'at' HH:mm")}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-white p-4 rounded-2xl shadow-sm space-y-1">
                ...
                >
                  {selectedRow.type === "STOCK_IN" ? "+" : "-"}
                  {Number(selectedRow.quantity).toFixed(0)} units
                </p>
              </div>
              <div className="bg-white p-4 rounded-2xl shadow-sm space-y-1">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                ...
                <p className=" font-black text-slate-800">
                  {selectedRow.reference || "Internal Sync"}
                </p>
              </div>
              {selectedRow.notes && (
                <div className="bg-white p-4 rounded-2xl shadow-sm space-y-1 col-span-2">
...
export function PurchaseHistoryReport({
  companyId,
  dateRange,
  search,
}: ReportProps) {
  ...
  const { data, isLoading } = useQuery<any[]>({
    queryKey: [
      api.reports.purchaseHistory.path,
      companyId,
      dateRange.from,
      dateRange.to,
    ],
                ...
                <td className="px-6 py-3">
                  <div className="flex flex-col">
                    <span className="font-bold text-slate-800 line-clamp-1">
                      {row.supplierName || "Direct / Internal"}
                    </span>
      ...
      <div className="flex-1 overflow-auto p-6 bg-slate-50/30">
        {!selectedRow ? (
          <div className="flex flex-col items-center justify-center h-full text-slate-300 gap-3">
            ...
            <p className=" font-medium">
              Select a purchase record to view costing
            </p>
          </div>
        ) : (
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className=" font-bold text-slate-500 uppercase tracking-tighter">
                  {format(new Date(selectedRow.date), "dd MMMM yyyy")}
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-white p-4 rounded-2xl shadow-sm space-y-1 col-span-2">
                ...
                <p className=" font-black text-slate-800">
                  {selectedRow.supplierName || "General Vendor"}
                </p>
              </div>
              <div className="bg-white p-4 rounded-2xl shadow-sm space-y-1 col-span-2">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              ...
              <div className="bg-white p-5 rounded-2xl shadow-sm space-y-1 col-span-2 border-l-8 border-emerald-500 bg-emerald-50/20">
                ...
                <p className="text-3xl font-black text-emerald-800">
                  ${Number(selectedRow.totalCost).toFixed(2)}
                </p>
              </div>
              <div className="bg-white p-4 rounded-2xl shadow-sm space-y-1 col-span-2">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
