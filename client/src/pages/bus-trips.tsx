...
function CreateTripDialog({ companyId }: { companyId: number }) {
    ...
    const conductors = Array.isArray(usersResponse) 
        ? usersResponse.filter((u: any) => u.role === "bus_conductor" || u.role === "cashier" || u.role === "admin")
        : [];
                ...
                <Form {...form}>
                    <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-slate-700 font-semibold">Route</FormLabel>
                                    <Select 
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-slate-700 font-semibold">Vehicle</FormLabel>
                                    <Select 
                                        onValueChange={v => field.onChange(parseInt(v))} 
                                        defaultValue={field.value ? field.value.toString() : ""}
                                    >
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-slate-700 font-semibold">Conductor</FormLabel>
                                    <Select 
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-slate-700 font-semibold">Departure Time</FormLabel>
                                    <FormControl>
                                        <Input 
                                            type="datetime-local" 
                        ...
                        <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                            ...
                            <Button type="submit" disabled={createTrip.isPending} className="rounded-xl bg-orange-600 hover:bg-orange-700 text-white">
...
export default function BusTripsPage() {
    ...
    const getStatusColor = (status: string) => {
        switch(status) {
            ...
            case 'boarding': return 'bg-amber-50 text-amber-700 border-amber-100';
            case 'en_route': return 'bg-purple-50 text-purple-700 border-purple-100';
                ...
                <CardContent className="p-0">
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-50">
                            {isLoading ? (
                                <tr><td colSpan={5} className="h-32 text-center text-slate-400">Loading trips...</td></tr>
                            ) : filteredTrips?.length === 0 ? (
                                <tr><td colSpan={5} className="h-32 text-center text-slate-400">No trips found</td></tr>
                                    ...
                                    <tr key={t.id} className="group hover:bg-slate-50/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col gap-1">
                                                <div className="text-sm font-bold text-slate-700 flex items-center gap-1.5 uppercase">
                                                    <Bus className="w-4 h-4 text-slate-400" />
                                                    {vehicle?.registrationNumber || `Vehicle #${t.vehicleId}`}
                                                </div>
                                                <div className="text-[11px] text-slate-500 font-medium flex items-center gap-1">
                                                    <Users className="w-3 h-3 text-slate-400" />
                                                    {vehicle?.capacity ? `${vehicle.capacity} Seats` : 'Unknown Capacity'}
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <div className="flex flex-col gap-1">
                                                <div className="text-sm font-semibold text-slate-700 flex items-center gap-1.5">
                                                    ...
                                                    {format(new Date(t.scheduledDeparture), "MMM d, yyyy")}
                                                </div>
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide shadow-sm border ${getStatusColor(t.status)}`}>
                                                {t.status.replace('_', ' ')}
                                            </span>
                                        </td>
