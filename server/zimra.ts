...
// LEKUKA gateways — on the `lekuka` branch (COUNTRY_SCOPE=Lesotho) every
// fiscal call must hit RSL, not ZIMRA. Mirrors shared/lekuka.ts constants
// so getZimraBaseUrl remains the single fiscal gateway resolver.
const LEKUKA_TEST_GATEWAY = 'https://lekukaapi.rsl.org.ls:8443';
const LEKUKA_PROD_GATEWAY = 'https://lekukaapi.rsl.org.ls';

/**
 * Get the appropriate fiscal gateway base URL based on environment.
 * On the `lekuka` branch (COUNTRY_SCOPE=Lesotho) this resolves to the RSL
 * LEKUKA gateway so *all* endpoints hit lekuka URL rather than ZIMRA.
