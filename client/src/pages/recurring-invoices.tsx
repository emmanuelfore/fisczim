...
export default function RecurringInvoicesPage() {
  ...
  const { data: recurring, isLoading } =
    useRecurringInvoices(selectedCompanyId);
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr>
                  ...
                  <th className="hidden lg:table-cell data-table-header w-[140px]">
                    Last Run
                  </th>
                  <th className="hidden md:table-cell data-table-header w-[140px]">
                    Next Run
                  </th>
                    ...
                    <td colSpan={6} className="p-12 text-center text-slate-500">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <p className="text-xs mt-1 text-slate-400">
                          Convert an existing invoice to a recurring schedule to
                          get started.
                        </p>
                      </div>
                    </td>
                  ...
                  filteredRecurring?.map((r) => (
                    <tr key={r.id} className="data-table-row group">
                      ...
                      <td className="hidden lg:table-cell data-table-cell text-slate-500">
                        {r.lastRunDate
                          ? format(new Date(r.lastRunDate), "dd MMM yyyy")
                          : "Never"}
                      </td>
                      <td className="hidden md:table-cell data-table-cell font-medium text-primary">
                        {r.nextRunDate
                          ? format(new Date(r.nextRunDate), "dd MMM yyyy")
                          : "-"}
                      </td>
                      <td className="data-table-cell">
                        <Badge
                      ...
                      <td className="data-table-cell text-right pr-4">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem
                              onClick={() =>
                                handleToggleStatus(r.id, r.status || "active")
                              }
                            >
                              {r.status === "active" ? (
                                <>
