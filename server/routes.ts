...
export async function registerRoutes(
  ...
  app.post("/api/companies/:companyId/maintenance/clear-data", requireOwner, async (req, res) => {
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        ...
        await tx.delete(zimraLogs).where(eq(zimraLogs.companyId, companyId));
        
        // Delete invoice-linked records
        if (invoiceIds.length > 0) {
          await tx.delete(validationErrors).where(sql`${validationErrors.invoiceId} IN ${invoiceIds}`);
          await tx.delete(payments).where(sql`${payments.invoiceId} IN ${invoiceIds}`);
          await tx.delete(invoiceItems).where(sql`${invoiceItems.invoiceId} IN ${invoiceIds}`);
          await tx.delete(fiscalizationJobs).where(sql`${fiscalizationJobs.invoiceId} IN ${invoiceIds}`);
        }

        // Delete POS Shift Transactions
        if (shiftIds.length > 0) {
          await tx.delete(posShiftTransactions).where(sql`${posShiftTransactions.shiftId} IN ${shiftIds}`);
        }

        // ── Step 3: Delete parent records ──
        await tx.delete(invoices).where(eq(invoices.companyId, companyId));
        await tx.delete(posShifts).where(eq(posShifts.companyId, companyId));
        await tx.delete(posHolds).where(eq(posHolds.companyId, companyId));
        await tx.delete(inventoryTransactions).where(eq(inventoryTransactions.companyId, companyId));
        ...
        await tx.update(products).set({ stockLevel: "0.00" }).where(eq(products.companyId, companyId));
  ...
  app.post("/api/companies/:id/lekuka/environment", requireAuthOrApiKey, async (req, res) => {
    try {
      ...
      const { environment } = req.body;
      if (!environment || !['test', 'production'].includes(environment)) {
        return res.status(400).json({ message: "Invalid environment. Must be 'test' or 'production'" });
      }
      const company: any = await storage.getCompany(companyId);
      if (!company) return res.status(404).json({ message: "Company not found" });
      if (company.fiscalDayOpen) {
        return res.status(400).json({ message: "Cannot switch environment while fiscal day is open", currentEnvironment: company.zimraEnvironment, fiscalDayNo: company.currentFiscalDayNo });
      }
      if (environment === 'production') {
        const macAddress = company.registeredMacAddress || "";
        const hasSub = await storage.hasActiveSubscriptionByMac(companyId, macAddress);
      ...
      // If the device isn't registered on the new env yet, mappings stay
      // stale and tax-health will flag them — the switch itself is not blocked.
  ...
  // ZIMRA Transaction Logs
  app.get("/api/companies/:id/zimra/logs", requireAuthOrApiKey, async (req, res) => {
    try {
      const companyId = req.params.id ? Number(req.params.id) : (req as any).apiKeyCompanyId;
  ...
  app.get("/api/companies/:id/zimra/sequence-report", requireAuthOrApiKey, async (req, res) => {
    try {
      const companyId = req.params.id ? Number(req.params.id) : (req as any).apiKeyCompanyId;
      const { db } = await import("./db.js");
  ...
  app.get(api.invoices.orderStatus.path, requireAuthOrApiKey, async (req, res) => {
    try {
      const companyId = parseInt(req.params.companyId);
  ...
  app.post("/api/companies/:companyId/lekuka/verify-taxpayer", requireAuth, async (req, res) => {
    try {
      ...
      const info = await device.verifyTaxpayerInformation(
        activationKey ? String(activationKey).trim() : undefined,
        deviceSerialNo ? String(deviceSerialNo).trim() : undefined,
      );
      res.json(info);
    } catch (err: any) {
  ...
  app.post("/api/companies/:companyId/lekuka/register", requireAuth, async (req, res) => {
    try {
      ...
      const { deviceId, activationKey, deviceSerialNo, gatewayUrl } = req.body || {};
      if (!deviceId || !activationKey || !deviceSerialNo) {
        return res.status(400).json({ message: "deviceId, activationKey and deviceSerialNo are required" });
      }
      const company: any = await storage.getCompany(companyId);
      if (!company) return res.status(404).json({ message: "Company not found" });
      ...
      if (!certificate) throw new Error("RSL did not return a device certificate");
  ...
  app.get("/api/companies/:companyId/lekuka/status", requireAuth, async (req, res) => { try {
    ...
    if (!company.fdmsDeviceId) missing.push("RSL device ID");
    if (!company.fdmsDeviceSerialNo) missing.push("Device serial number");
    if (!company.zimraCertificate) missing.push("Device certificate");
    if (!company.zimraPrivateKey) missing.push("Private key");
    const isConfigured = missing.length === 0;
    res.json({
      isConfigured,
      isOnline: isConfigured, // registered device is considered online; live RSL GetStatus is on-demand via connectivity-test
      fiscalDayOpen: !!company.fiscalDayOpen,
      fiscalDayStatus: company.fiscalDayOpen ? "FiscalDayOpened" : "FiscalDayClosed",
      fiscalDayNumber: company.currentFiscalDayNo ?? null,
      lastSync: company.fiscalDayOpenedAt ?? null,
      hasDeviceId: !!company.fdmsDeviceId,
      hasSerialNo: !!company.fdmsDeviceSerialNo,
      hasCertificate: !!company.zimraCertificate,
      hasPrivateKey: !!company.zimraPrivateKey,
      missing,
    });
  } catch (err: any) {
  ...
  app.get("/api/companies/:companyId/lekuka/tax-health", requireAuth, async (req, res) => {
    try {
      ...
      const [companyProducts] = [await db.select({ id: products.id, name: products.name, taxTypeId: products.taxTypeId }).from(products).where(eq(products.companyId, companyId))];
    ...
    try {
      ...
      for (const currency of activeCurrencies) {
        ...
        const basePayload = {
          ...
          buyerTin: (testCustomer as any).tin ?? null,
          buyerVat: (testCustomer as any).vatNumber ?? null,
          customerPhone: (testCustomer as any).phone ?? null,
          deliveryAddress: (testCustomer as any).address ?? null,
          issueDate: new Date(),
          dueDate,
  ...
  app.post("/api/payments/paynow-update", async (req, res) => {
    try {
      ...
      // TODO: Add HMAC signature verification when Paynow provides signing keys
      await paynowService.checkStatus(reference);
      res.status(200).end();
    } catch (error: any) {
      console.error("[Paynow] Webhook error:", error);
      res.status(200).end(); // Return 200 to prevent retries from Paynow
    }
  });
