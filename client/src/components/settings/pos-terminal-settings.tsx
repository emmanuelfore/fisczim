...
export function PosTerminalSettings({
  ...
  isLoading,
  companyId,
}: PosTerminalSettingsProps) {
  const { toast } = useToast();
  const [, setLocation] = useLocation();
  const queryClient = useQueryClient();
  const [activeSection, setActiveSection] = useState("terminal");
  const [printerClientStatus, setPrinterClientStatus] = useState<
  ...
  const cashiers =
    users?.filter(
      (u: any) =>
        u.role === "cashier" || u.role === "admin" || u.role === "owner",
    ) || [];
  ...
  const addCashierMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Cashier Added",
        description: `Added ${newName} successfully.`,
      });
  ...
  const updatePinMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await apiFetch(
        `/api/companies/${companyId}/users/${selectedUserForPin.id}/pin`,
        {
          method: "PUT",
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[240px_1fr] gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <aside className="space-y-1">
        ...
        {sections.map((s) => (
          ...
          >
            <s.icon
              className={`w-4 h-4 ${activeSection === s.id ? "text-white" : "text-slate-400"}`}
            />
            {s.label}
          </button>
        ))}
              ...
              <CardContent className="pt-6 space-y-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updatePosSetting("terminalId", e.target.value)
                    }
                    placeholder="e.g. T-001, FRONT-DESK-1"
                  />
                  <p className="text-[11px] text-slate-400 italic">
                    This ID helps track which physical device issued a specific
                    receipt.
                  </p>
                </div>
              </CardContent>
              ...
              <CardContent className="pt-6 space-y-6">
                    ...
                    <div className="flex gap-2">
                      {["80mm", "58mm", "A4"].map((size) => (
                        <button
                          key={size}
                          onClick={() =>
                            updatePosSetting("receiptPaperSize", size)
                          }
                  ...
                  <div className="flex items-center gap-3 pt-6">
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        updatePosSetting("receiptShowLogo", checked)
                      }
                    />
                    <Label
                      htmlFor="showLogo"
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updatePosSetting("receiptHeader", e.target.value)
                    }
                ...
                <div className="space-y-2">
                  ...
                  <textarea
                    className="w-full min-h-[100px] rounded-xl border border-slate-200 p-3  focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500"
                    value={posSettings.receiptFooter || ""}
                    onChange={(e) =>
                      updatePosSetting("receiptFooter", e.target.value)
                    }
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-6">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <div
                  className={`w-2 h-2 rounded-full ${printerClientStatus === "online" ? "bg-green-500" : "bg-red-500"}`}
                />
                <span className="text-[10px] font-black uppercase text-slate-500">
                  Client: {printerClientStatus}
                </span>
              </div>
            </div>
                ...
                <div className="flex items-center justify-between p-4 bg-blue-50/50 rounded-2xl border border-blue-100 mb-4">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-blue-600/70">
                      Master toggle — disables all receipt printing when off.
                    </p>
                  </div>
                  <Switch
                    checked={posSettings.printingEnabled ?? true}
                    onCheckedChange={(checked) =>
                      updatePosSetting("printingEnabled", checked)
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-slate-500">
                      Print immediately after sale completion.
                    </p>
                  </div>
                  <Switch
                    checked={posSettings.autoPrint}
                    onCheckedChange={(checked) =>
                      updatePosSetting("autoPrint", checked)
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label className=" font-bold text-indigo-900">
                      Use Local Printer Client
                    </Label>
                    <p className="text-xs text-indigo-600/70">
                      Connect to physical thermal printers via Desktop App.
                    </p>
                  </div>
                  <Checkbox
                    checked={posSettings.usePrinterClient}
                    onCheckedChange={(checked) =>
                      updatePosSetting("usePrinterClient", checked)
                    }
                  />
                </div>
                  ...
                  <div className="bg-white/80 rounded-2xl p-4 border border-indigo-100 mt-2 space-y-3">
                    <div className="flex items-center justify-between group">
                      <Label className="text-xs font-bold text-slate-700">
                        Silent Printing (Proxy)
                      </Label>
                      <Switch
                        checked={posSettings.silentPrinting}
                        onCheckedChange={(v) =>
                          updatePosSetting("silentPrinting", v)
                        }
                      />
                    </div>
                    ...
                    {posSettings.silentPrinting && (
                      <div className="space-y-3 pt-2 animate-in fade-in slide-in-from-top-2">
                        <div className="space-y-1.5">
                          ...
                          <Input
                            value={posSettings.printServerUrl}
                            onChange={(e) =>
                              updatePosSetting("printServerUrl", e.target.value)
                            }
                        ...
                        <div className="space-y-1.5">
                          <div className="flex justify-between items-center">
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              className="h-6 px-2 text-[10px] hover:bg-indigo-50 text-indigo-600"
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Select
                            value={
                              posSettings.secondaryPrinterName || "disabled"
                            }
                            onValueChange={(val) =>
                              updatePosSetting(
                                "secondaryPrinterName",
                    ...
                    <div className="flex gap-2 pt-2">
                      <Button
                        ...
                        onClick={() =>
                          window.open("/downloads/printer-client.zip")
                        }
                      >
                          ...
                          <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                            <div className="space-y-3">
                              ...
                              <p className=" text-slate-600 pl-8">
                                Download <code>printer-client.zip</code>,
                                extract it, and run{" "}
                                <code>printer-client.exe</code>.
                              </p>
                            </div>
                            <div className="space-y-3">
                              <h4 className="font-bold text-slate-900 flex items-center gap-2 bg-slate-50 p-2 rounded-lg">
                              ...
                              <p className=" text-slate-600 pl-8">
                                In the client, select your thermal printer
                                (e.g., POS-80) and click{" "}
            ...
            <Card className="card-depth border-none">
              <CardContent className="pt-6 space-y-6">
                <div className="space-y-2">
                  ...
                  <Select
                    value={posSettings.defaultCustomerId?.toString() || "0"}
                    onValueChange={(v) =>
                      updatePosSetting("defaultCustomerId", v)
                    }
                  >
                  ...
                  <p className="text-[11px] text-slate-400">
                    Automatically selected when starting a new sale.
                  </p>
                </div>
                ...
                <div className="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl border border-slate-100">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-slate-500">
                      Allow adding items with 0 or negative stock level to cart.
                    </p>
                  </div>
                  <Switch
                    checked={posSettings.allowSellOutOfStock}
                    onCheckedChange={(v) =>
                      updatePosSetting("allowSellOutOfStock", v)
                    }
                  />
                </div>
                ...
                <div className="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl border border-slate-100">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-slate-500">
                      Number of decimal places allowed for item quantities (e.g.
                      2 for meat/weight).
                    </p>
                  </div>
                  <div className="w-24">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 sm:grid-cols-3 gap-2">
                    {[
                      ...
                      >
                        {posSettings.allowedPaymentMethods?.includes(
                          method.id,
                        ) ? (
                  ...
                  {[
                    {
                      ...
                      desc: "Require admin PIN when applying a manual discount.",
                    },
                    {
                      id: "PriceChange",
                      label: "Price Change",
                      desc: "Require admin PIN when manually changing price.",
                    },
                    {
                      id: "Delete",
                      label: "Delete Item / Void",
                      desc: "Require admin PIN to remove items or void.",
                    },
                    {
                      id: "OpenDrawer",
                    ...
                    <div
                      key={rule.id}
                      className="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl border border-slate-100 group"
                    >
                      <div className="space-y-0.5">
                        <Label className=" font-bold text-slate-700">
                          {rule.label}
        ...
        {activeSection === "barcodes" && (
          <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-6">
            ...
            <p className="text-xs text-slate-500 bg-slate-50 p-3 rounded-xl border border-slate-100">
              Define how to parse barcodes from weighing scales. When a barcode
              matches a rule's prefix and length, the system extracts the
              product SKU and quantity automatically without any manual input.
            </p>
            ...
            {(posSettings.variableWeightBarcodeRules || []).map(
              (rule: any, idx: number) => {
                const updateRule = (key: string, value: any) => {
                  const rules = [
                    ...(posSettings.variableWeightBarcodeRules || []),
                  ];
                ...
                const previewLen =
                  Number(rule.prefix?.length || 0) +
                  Number(rule.skuLength || 0) +
                  Number(rule.quantityLength || 0) +
                  1;

                return (
                  <Card
                    key={rule.id}
                    className={`card-depth border-none transition-all ${rule.enabled ? "ring-1 ring-violet-200" : "opacity-60"}`}
                  >
                    <CardContent className="pt-5 space-y-4">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Input
                          ...
                          className="h-8  font-bold border-none bg-slate-50 rounded-xl flex-1"
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 sm:grid-cols-3 gap-3">
                        <div className="space-y-1">
                          ...
                          <Input
                            value={rule.prefix}
                            onChange={(e) =>
                              updateRule("prefix", e.target.value)
                            }
                            className="h-9  font-mono font-bold"
                            placeholder="20"
                          />
                          <p className="text-[9px] text-slate-400">
                            Identifies this barcode type
                          </p>
                        </div>
                        <div className="space-y-1">
                          <Label className="text-[10px] font-black uppercase text-slate-400">
                      ...
                      <div className="bg-violet-50/50 rounded-xl p-3 border border-violet-100 space-y-2">
                        ...
                        <div className="flex items-center gap-1 flex-wrap font-mono text-xs">
                          <span className="px-1.5 py-0.5 bg-violet-200 text-violet-800 rounded font-black">
                            {rule.prefix || "??"}
                          </span>
                          <span className="text-slate-300 text-[10px]">
                            prefix
                          </span>
                          <span className="px-1.5 py-0.5 bg-emerald-100 text-emerald-700 rounded font-black">
                            {"X".repeat(rule.skuLength || 0)}
                          </span>
                          <span className="text-slate-300 text-[10px]">
                            sku
                          </span>
                          <span className="px-1.5 py-0.5 bg-blue-100 text-blue-700 rounded font-black">
                            {"Y".repeat(rule.quantityLength || 0)}
                          </span>
                          <span className="text-slate-300 text-[10px]">
                            qty
                          </span>
                          <span className="px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded font-black">
                            Z
                          </span>
                          <span className="text-slate-300 text-[10px]">
                            check
                        ...
                        <div className="pt-1 border-t border-violet-100 space-y-1.5">
                          ...
                          <QuantityInput
                            ...
                            onChange={(e) => {
                              ...
                              const matchesPrefix = testBarcode.startsWith(
                                rule.prefix || "",
                              );
                              const matchesLen =
                                testBarcode.length === (rule.totalLength || 0);
                              ...
                              const sku = testBarcode.substring(
                                rule.skuStart || 0,
                                (rule.skuStart || 0) + (rule.skuLength || 0),
                              );
                              const qtyRaw = parseInt(
                                testBarcode.substring(
                                  rule.quantityStart || 0,
                                  (rule.quantityStart || 0) +
                                    (rule.quantityLength || 0),
                                ),
                              );
                              const qty =
                                qtyRaw / (rule.quantityDivisor || 1000);
                          ...
                          />
                          <p
                            id={`barcode-test-result-${rule.id}`}
              ...
              <DialogContent className="rounded-[2rem]">
                ...
                <DialogFooter>
                  ...
                  <Button
                    ...
                    onClick={() => addCashierMutation.mutate()}
              ...
              <DialogContent className="max-w-xs rounded-[2rem]">
                <DialogHeader>
                  <DialogTitle className="text-center">
                    Set PIN for {selectedUserForPin?.name}
                  </DialogTitle>
                </DialogHeader>
                <div className="py-6">
                  <Input
        ...
        {activeSection === "restaurant" && (
          <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-6">
              ...
              <CardContent className="pt-6 space-y-6">
                <div className="flex items-center justify-between p-4 bg-white/80 rounded-2xl border border-indigo-100">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-xs text-slate-500">
                      Enable table management, guest counts, and waiter
                      selection in POS.
                    </p>
                  </div>
                  <Switch
                ...
                {formData.restaurantSettings?.enabled && (
                  <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                    <div className="flex items-center justify-between p-4 bg-white/80 rounded-2xl border border-slate-100">
                      <div className="space-y-0.5">
                        <Label className=" font-bold text-slate-700">
                          Require Waiter Selection
                        </Label>
                        <p className="text-xs text-slate-500">
                          Force staff to select a waiter before starting an
                          order.
                        </p>
                      </div>
                      <Checkbox
                        checked={
                          formData.restaurantSettings?.requireWaiterSelection ??
                          false
                        }
                        onCheckedChange={(checked) =>
                          setFormData({
                    ...
                    <div className="flex items-center justify-between p-4 bg-white/80 rounded-2xl border border-slate-100">
                      <div className="space-y-0.5">
                        <Label className=" font-bold text-slate-700">
                          Allow Multi-Dining Options
                        </Label>
                        <p className="text-xs text-slate-500">
                          Enable Dine-In, Take-Away, and Delivery selection.
                        </p>
                      </div>
                      <Checkbox
                        checked={
                          formData.restaurantSettings?.allowDiningOptions ??
                          true
                        }
                        onCheckedChange={(checked) =>
                          setFormData({
                    ...
                    <div className="flex items-center justify-between p-4 bg-indigo-600/5 rounded-2xl border border-indigo-200 group">
                      <div className="space-y-0.5">
                        <Label className=" font-black text-indigo-900">
                          Floor Plan & Table Layout
                        </Label>
                        <p className="text-xs text-indigo-700/70">
                          Visual layout for your dining area.
                        </p>
                      </div>
                      <Button
                        size="sm"
            ...
            <div className="bg-amber-50 p-4 rounded-2xl border border-amber-100 flex gap-3">
              ...
              <div className="space-y-1">
                <p className="text-xs font-bold text-amber-900 tracking-tight leading-tight">
                  Switching Modes?
                </p>
                <p className="text-[10px] text-amber-800/80 leading-relaxed">
                  Enabling restaurant mode will change the POS landing screen
                  from a product grid to a <strong>Table Perspective</strong>.
                  Ensure your staff is trained on table-based ordering before
                  switching in a production environment.
                </p>
              </div>
            </div>
          </div>
              ...
              <Card className="card-depth border-none bg-slate-900 text-white overflow-hidden group">
                <CardContent className="pt-6 space-y-4">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="px-3 py-1 bg-indigo-500 rounded-full text-[10px] font-black uppercase tracking-wider">
                      Desktop
                    </div>
                  </div>
                  <div>
                    ...
                    <p className="text-slate-400 text-xs">
                      Run your POS faster with the native desktop application.
                    </p>
                  </div>
                  <div className="flex flex-col gap-2">
                    <Button
                      ...
                      onClick={() =>
                        window.open(
                          "https://github.com/FiscalStack/fisczim/releases/latest",
                        )
                      }
                    >
                      <ExternalLink className="w-4 h-4" /> Windows Installer
                      (.exe)
                    </Button>
                    <Button
                      variant="ghost"
              ...
              <Card className="card-depth border-none border border-slate-100 group">
                <CardContent className="pt-6 space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 text-xs text-pretty">
                      Lightweight service for silent thermal printing & cash
                      drawers.
                    </p>
                  </div>
                  <Button
