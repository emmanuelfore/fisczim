...
export default function CreditorAnalysisPage() {
  ...
  if (!analysis) return <Layout><div>Creditor analysis not found.</div></Layout>;
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-bold text-slate-800 font-display">Creditor Analysis: {supplier.name}</h2>
              <p className="text-sm text-slate-500">Accounts payable and supplier liability audit</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
           <Card className="border-slate-200/60 shadow-sm rounded-xl overflow-hidden">
             <CardContent className="p-4">
               ...
               <p className="text-[10px] text-slate-500 mt-1 font-medium">{metrics.invoiceCount} Pending Supplier Invoices</p>
             </CardContent>
           </Card>
           ...
           <Card className="border-slate-200/60 shadow-sm rounded-xl overflow-hidden">
             <CardContent className="p-4">
               ...
               <p className="text-[10px] text-slate-500 mt-1 font-medium">Life-to-date purchase volume</p>
             </CardContent>
           </Card>
           ...
           <Card className="border-slate-200/60 shadow-sm rounded-xl overflow-hidden">
             <CardContent className="p-4">
               ...
               <p className="text-[10px] text-slate-500 mt-1 font-medium">Most recent outflow to supplier</p>
             </CardContent>
           </Card>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                 ...
                 <Table>
                    <TableHeader className="bg-slate-50/50">
                       <TableRow className="border-slate-50">
                          ...
                          <TableHead className="text-right pr-6 font-bold text-[10px] text-slate-500 tracking-widest uppercase">Aging</TableHead>
                       </TableRow>
                    </TableHeader>
                    <TableBody>
                       {invoices.length === 0 ? (
                          <TableRow><TableCell colSpan={4} className="h-40 text-center text-slate-400 italic">No open payables</TableCell></TableRow>
                       ) : (
                          invoices.map((inv) => (
                             <TableRow key={inv.id} className="group hover:bg-slate-50/50 transition-colors border-slate-50">
                                <TableCell className="pl-6">
                                   <div className="flex flex-col">
                                      ...
                                      <span className="text-[10px] text-slate-400 font-medium">Dated: {format(new Date(inv.date), "MMM dd, yyyy")}</span>
                                   </div>
                                </TableCell>
                                <TableCell className="text-center">
                                   <div className="text-[12px] font-medium text-slate-700">
                                      {format(new Date(inv.dueDate || inv.date), "MMM dd, yyyy")}
                                   </div>
                                </TableCell>
                                ...
                                <TableCell className="text-right pr-6">
                                   {inv.daysOverdue > 0 ? (
                                      <Badge variant="destructive" className={cn(
                                         ...
                                         inv.daysOverdue > 30 ? "bg-rose-600" : "bg-orange-500"
                                      )}>
                                         {inv.daysOverdue} Days Late
                                      </Badge>
                                   ) : (
                                      <Badge variant="outline" className="bg-slate-50 text-slate-500 border-slate-200 rounded-md text-[10px] px-1.5 py-0.5">
                                         Current
                                      </Badge>
                                   )}
                                </TableCell>
           ...
           <div className="space-y-6">
              <Card className="border-slate-200/60 shadow-sm rounded-2xl bg-white overflow-hidden">
                 <CardHeader className="border-b border-slate-50 py-4 px-6">
                    <CardTitle className="text-sm font-bold text-slate-800 uppercase tracking-tight">Supplier Terms</CardTitle>
                 </CardHeader>
                 <CardContent className="p-6 space-y-4">
                    <div className="space-y-1">
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Supplier TIN</span>
                       <p className="text-sm font-bold text-slate-700">{supplier.tin || "Not Provided"}</p>
                    </div>
                    <div className="space-y-1">
                       <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Contact Details</span>
                       <p className="text-sm font-bold text-slate-700">{supplier.phone || "No phone"}</p>
                       <p className="text-xs text-slate-500 font-medium">{supplier.email || "No email"}</p>
                    </div>
                    <div className="space-y-1 mt-4">
                       <div className="bg-slate-50 rounded-lg p-3">
                          <div className="flex justify-between items-center mb-1">
                             <span className="text-[10px] font-bold text-slate-500 uppercase">Payment Terms:</span>
                             <span className="text-sm font-black text-slate-800">{supplier.creditDays || 0} Days</span>
                          </div>
                          <div className="flex justify-between items-center">
                             <span className="text-[10px] font-bold text-slate-500 uppercase">Credit Limit:</span>
                             <span className="text-sm font-black text-slate-800">{formatCurrency(supplier.creditLimit)}</span>
                          </div>
                       </div>
                    </div>
                 </CardContent>
              ...
              <Card className="border-slate-200 shadow-sm rounded-2xl bg-slate-50/30 overflow-hidden">
                 <CardContent className="p-6">
                    <h4 className="text-sm font-black text-slate-800 uppercase tracking-tight mb-2">Liquidity Sentiment</h4>
                    <p className="text-xs text-slate-600 font-medium leading-relaxed">
                       This supplier accounts for {metrics.totalPurchases > 0 ? Math.round((metrics.totalOutstanding / metrics.totalPurchases) * 100) : 0}% of historical engagement currently outstanding. 
                       Maintain standard terms to preserve supply chain stability.
                    </p>
                 </CardContent>
              </Card>
           </div>
