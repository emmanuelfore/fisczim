...
# Fetch Tax Types
tax_fetch_target = """  const { data: accounts = [] } = useQuery<Account[]>({"""
tax_fetch_replacement = """  const { data: taxTypes = [] } = useQuery<any[]>({
...
# Submit payload
submit_payload_target = """      const response = await apiFetch(`/api/companies/${companyId}/gdns`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          supplierId: Number(supplierId),
          gdnNumber: gdnNumber || undefined,
          notes: notes || null,
          items,
        }),
      });"""
submit_payload_replacement = """      const response = await apiFetch(`/api/companies/${companyId}/gdns`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          supplierId: Number(supplierId),
...
table_header_target = """                  <div className="grid grid-cols-12 gap-0 bg-slate-50 px-3 py-2 text-[10px] font-black uppercase tracking-widest text-slate-400">
                    <div className="col-span-5">Product / Expense</div>
...
table_cost_target = """                        <div className="col-span-2">
                        ...
                        <div className="col-span-2 text-right text-base font-bold text-slate-800 flex items-center justify-end h-9">
                          {currency} {(Number(line.quantity || 0) * Number(line.unitCost || 0)).toFixed(2)}
                        </div>"""
                          ...
                          {line.taxTypeId && line.taxTypeId !== "__none__" && (
                            <div className="flex items-center gap-1">
                              <input 
                                ...
                                className="h-3 w-3 rounded text-emerald-600 focus:ring-emerald-500 border-gray-300"
                              />
...
bottom_totals_target = """                <div className="pt-6 mt-6 border-t border-slate-100 flex justify-end">
                  <div className="text-right">
                    ...
                    <p className="text-3xl font-black text-emerald-600">{currency} {total.toFixed(2)}</p>
                  </div>
                </div>"""
                  ...
                  <div className="text-right min-w-[250px]">
                    ...
                    {totalTax > 0 && (
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="font-bold">{currency} {totalTax.toFixed(2)}</span>
                      </div>
                    )}
