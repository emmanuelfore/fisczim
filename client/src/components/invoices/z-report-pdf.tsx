...
try {
  Font.register({
    ...
    fonts: [
      {
        src: "https://cdn.jsdelivr.net/npm/@fontsource/courier-prime@4.5.5/files/courier-prime-latin-400-normal.woff",
        fontWeight: 400,
      },
      {
        src: "https://cdn.jsdelivr.net/npm/@fontsource/courier-prime@4.5.5/files/courier-prime-latin-700-normal.woff",
        fontWeight: 700,
      },
    ],
  });
...
export const ZReportPDF = ({ data, isZReport }: ZReportPDFProps) => {
  ...
  const getCountersByType = (curr: string, type: string) => {
    return (counters || [])
      .filter(
        (c: any) =>
          c.fiscalCounterCurrency === curr && c.fiscalCounterType === type,
      )
      .sort(
        (a: any, b: any) =>
          (b.fiscalCounterTaxPercent || 0) - (a.fiscalCounterTaxPercent || 0),
      );
  };
      ...
      <Page size="A4" style={[styles.page, { fontFamily: pageFont }]}>
        ...
        <View style={styles.header}>
          ...
          {company.branchName && company.branchName !== company.name && (
            <Text>Branch: {company.branchName}</Text>
          )}
        ...
        <View style={styles.section}>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.value}>
              {openedAt
                ? format(new Date(openedAt), "dd/MM/yyyy HH:mm")
                : "N/A"}
            </Text>
          </View>
          <View style={styles.row}>
            ...
            <Text style={styles.value}>
              {isZReport && closedAt
                ? format(new Date(closedAt), "dd/MM/yyyy HH:mm")
                : isZReport
                  ? "N/A"
                  : "STILL OPEN"}
            </Text>
          ...
          <View style={styles.row}>
            ...
            <Text style={styles.value}>
              {company.fdmsDeviceSerialNo || "N/A"}
            </Text>
          </View>
          <View style={styles.row}>
        ...
        {docStats &&
          docStats.map((stats: any) => {
            ...
            const cnTaxCounters = getCountersByType(
              currency,
              "CreditNoteTaxByTax",
            );

            const dnCounters = getCountersByType(currency, "DebitNoteByTax");
            const dnTaxCounters = getCountersByType(
              currency,
              "DebitNoteTaxByTax",
            );
              ...
              <View key={currency} break={docStats.indexOf(stats) > 0}>
                ...
                <View style={styles.section}>
                  ...
                  {netCounters.map((c: any, i: number) => {
                    const taxC = netTaxCounters.find(
                      (tc: any) =>
                        tc.fiscalCounterTaxPercent ===
                        c.fiscalCounterTaxPercent,
                    );

                    const cnC = cnCounters.find(
                      (cc: any) =>
                        cc.fiscalCounterTaxPercent ===
                        c.fiscalCounterTaxPercent,
                    );
                    const cnTaxC = cnTaxCounters.find(
                      (ctc: any) =>
                        ctc.fiscalCounterTaxPercent ===
                        c.fiscalCounterTaxPercent,
                    );

                    const dnC = dnCounters.find(
                      (dc: any) =>
                        dc.fiscalCounterTaxPercent ===
                        c.fiscalCounterTaxPercent,
                    );
                    const dnTaxC = dnTaxCounters.find(
                      (dtc: any) =>
                        dtc.fiscalCounterTaxPercent ===
                        c.fiscalCounterTaxPercent,
                    );

                    // Formula: (Sale - SaleTax) + (CN - CNTax) + (DN - DNTax)
                    const netVal =
                      c.fiscalCounterValue -
                      (taxC?.fiscalCounterValue || 0) +
                      ((cnC?.fiscalCounterValue || 0) -
                        (cnTaxC?.fiscalCounterValue || 0)) +
                      ((dnC?.fiscalCounterValue || 0) -
                        (dnTaxC?.fiscalCounterValue || 0));

                    return (
                      <View key={i} style={styles.row}>
                        <Text style={styles.label}>
                          Net, VAT {c.fiscalCounterTaxPercent}%
                        </Text>
                        <Text style={styles.value}>
                  ...
                  <View style={[styles.row, { marginTop: 4 }]}>
                    ...
                    <Text style={[styles.value, styles.bold]}>
                      {(
                        netCounters.reduce(
                          (s: number, c: any) => s + c.fiscalCounterValue,
                          0,
                        ) -
                        netTaxCounters.reduce(
                          (s: number, c: any) => s + c.fiscalCounterValue,
                          0,
                        ) +
                        (cnCounters.reduce(
                ...
                <View style={styles.section}>
                  ...
                  {netTaxCounters.map((c: any, i: number) => {
                    ...
                    return (
                      <View key={i} style={styles.row}>
                        <Text style={styles.label}>
                          Tax, VAT {c.fiscalCounterTaxPercent}%
                        </Text>
                        <Text style={styles.value}>
                          {taxVal.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}
                        </Text>
                ...
                <View style={styles.section}>
                  ...
                  {netCounters.map((c: any, i: number) => {
                    ...
                    return (
                      <View key={i} style={styles.row}>
                        <Text style={styles.label}>
                          Total, VAT {c.fiscalCounterTaxPercent}%
                        </Text>
                        <Text style={styles.value}>
                          {grossVal.toLocaleString(undefined, {
                            minimumFractionDigits: 2,
                            maximumFractionDigits: 2,
                          })}
                        </Text>
                ...
                <View style={styles.section}>
                  ...
                  <View style={styles.row}>
                    ...
                    <Text style={{ width: "40%", textAlign: "right" }}>
                      {stats.invoices.total.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </Text>
                  ...
                  <View style={styles.row}>
                    ...
                    <Text style={{ width: "40%", textAlign: "right" }}>
                      {stats.creditNotes.total.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </Text>
                  ...
                  <View style={styles.row}>
                    ...
                    <Text style={{ width: "40%", textAlign: "right" }}>
                      {stats.debitNotes.total.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </Text>
                  ...
                  <View
                    style={[
                      styles.row,
                      styles.bold,
                      { marginTop: 4, borderTopWidth: 1, paddingTop: 4 },
                    ]}
                  >
                    ...
                    <Text style={{ width: "40%", textAlign: "right" }}>
                      {stats.totalDocuments.total.toLocaleString(undefined, {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2,
                      })}
                    </Text>
        ...
        {data.posSummary && (
          <View style={styles.section}>
            ...
            <View style={[styles.row, styles.bold, { marginTop: 10 }]}>
              ...
              <Text style={styles.value}>
                {data.posSummary.totalTransactions}
              </Text>
            </View>
            <View style={styles.divider} />

            {data.posSummary.byPaymentMethod &&
              data.posSummary.byPaymentMethod.map((pm: any, i: number) => (
                <View key={i} style={styles.row}>
                  <Text style={styles.label}>
                    {pm.method} ({pm.count} sales):
                  </Text>
                  <Text style={styles.value}>
                    {Number(pm.total).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}
                  </Text>
        ...
        {data.posSummary?.posTransactions &&
          data.posSummary.posTransactions.length > 0 && (
            <View style={styles.section} break>
              ...
              >
                --- CASH MOVEMENTS (AUDIT TRAIL) ---
              </Text>
              <View
                style={[
                  styles.row,
                  styles.bold,
                  { marginTop: 10, borderBottomWidth: 1, paddingBottom: 4 },
                ]}
              >
              ...
              {data.posSummary.posTransactions.map((t: any, i: number) => (
                ...
                >
                  ...
                  <Text
                    style={{ width: "20%", textAlign: "center", fontSize: 8 }}
                  >
                    {t.userName}
                  </Text>
              ...
              <View
                style={[
                  styles.row,
                  styles.bold,
                  { marginTop: 10, paddingTop: 5, borderTopWidth: 1 },
                ]}
              >
