...
public class EMVCOHelper {
    ...
    public static int PayPass_ShowAmount() {
        ...
        if (Data_len > 0) {
            String PaypssTag_data2 = "";
            for (int i = 0; i < Data_len; i++) {
                PaypssTag_data2 = PaypssTag_data2 + ByteUtil.byteToHexString(PaypassTagBuff[i]);
            }
            int i2 = Data_len / 2;
            if (i2 != 0) {
                PaypssTag_data = PaypssTag_data2.substring(0, Data_len * 2);
            } else {
                PaypssTag_data = PaypssTag_data2;
            }
    ...
    public static int EmvPinbyPass() {
        ...
        return (TagPinbyPass_Data[2] & 8) == 1 ? 1 : 0;
    }
