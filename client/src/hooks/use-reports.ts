...
export function useFinancialSummary(companyId: number, from?: string, to?: string) {
    return useQuery({
        queryKey: [api.reports.financialSummary.path, companyId, from, to],
        queryFn: async () => {
            const url = buildUrl(api.reports.financialSummary.path, { companyId, from, to });
            const res = await apiFetch(url);
