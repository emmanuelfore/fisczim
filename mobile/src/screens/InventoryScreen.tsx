...
export function InventoryScreen({ onOpenDrawer, companyId }: Props) {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      setShowForm(false);
      refreshProducts();
      // Subtle feedback via modal closing
    } catch (e: any) {
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    const stockColor = stock <= 0 ? C.red : stock <= lowThreshold ? C.accent : C.green;
    return (
    ...
    <View style={styles.container}>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <View style={styles.searchRow}>
          ...
          <TextInput style={styles.searchInput} placeholder="Search products..." placeholderTextColor={C.muted} value={search} onChangeText={setSearch} />
        </View>
        {isLoading && !products ? (
            ...
            <View style={styles.modalContent}>
              ...
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 24 }}>
                ...
                <View style={styles.field}><Text style={styles.fieldLabel}>Barcode</Text>
                  <TextInput style={styles.fieldInput} value={form.barcode} onChangeText={(v) => setForm({ ...form, barcode: v })} placeholder="Optional" placeholderTextColor={C.muted} /></View>
                {/* HS Code */}
                <View style={styles.field}><Text style={styles.fieldLabel}>HS Code</Text>
                  <TextInput style={styles.fieldInput} value={form.hsCode} onChangeText={(v) => setForm({ ...form, hsCode: v })} placeholder="0000.00.00" placeholderTextColor={C.muted} /></View>
                {/* SKU */}
                <View style={styles.field}><Text style={styles.fieldLabel}>Code / SKU *</Text>
                  <TextInput style={styles.fieldInput} value={form.sku} onChangeText={(v) => setForm({ ...form, sku: v })} placeholderTextColor={C.muted} /></View>
                {/* Category */}
                <View style={styles.field}>
                  <Text style={styles.fieldLabel}>Category</Text>
                  <TextInput style={styles.fieldInput} value={form.category} onChangeText={(v) => setForm({ ...form, category: v })} placeholder="e.g. Beverages, Electronics" placeholderTextColor={C.muted} />
                  {categories.length > 0 && (
                ...
                <View style={{ flexDirection: "row", gap: 10 }}>
                  <View style={[styles.field, { flex: 1 }]}><Text style={styles.fieldLabel}>Selling Price *</Text>
                    <TextInput style={styles.fieldInput} keyboardType="numeric" value={form.price} onChangeText={(v) => setForm({ ...form, price: v })} placeholderTextColor={C.muted} /></View>
                  <View style={[styles.field, { flex: 1 }]}><Text style={styles.fieldLabel}>Cost Price</Text>
                    <TextInput style={styles.fieldInput} keyboardType="numeric" value={form.costPrice} onChangeText={(v) => setForm({ ...form, costPrice: v })} placeholderTextColor={C.muted} /></View>
                </View>
                ...
                {form.isTracked && (
                  <View style={{ flexDirection: "row", gap: 10 }}>
                    <View style={[styles.field, { flex: 1 }]}><Text style={styles.fieldLabel}>Opening Stock</Text>
                      <TextInput style={styles.fieldInput} keyboardType="numeric" value={form.stockLevel} onChangeText={(v) => setForm({ ...form, stockLevel: v })} placeholderTextColor={C.muted} /></View>
                    <View style={[styles.field, { flex: 1 }]}><Text style={styles.fieldLabel}>Low Stock Alert</Text>
                      <TextInput style={styles.fieldInput} keyboardType="numeric" value={form.lowStockThreshold} onChangeText={(v) => setForm({ ...form, lowStockThreshold: v })} placeholderTextColor={C.muted} /></View>
                  </View>
                )}
                {/* Description */}
                <View style={styles.field}><Text style={styles.fieldLabel}>Description</Text>
                  <TextInput style={[styles.fieldInput, { height: 70, textAlignVertical: "top" }]} multiline value={form.description} onChangeText={(v) => setForm({ ...form, description: v })} placeholderTextColor={C.muted} /></View>
...
const styles = StyleSheet.create({
  ...
  cardPrice: { color: C.text, fontSize: 13, fontWeight: "800" },
  cardStock: { color: C.green, fontSize: 10, marginTop: 2, fontWeight: "600" },
  errorText: { color: C.red, textAlign: "center", marginTop: 40 },
