...
export default function CreateQuotationPage() {
  ...
  const { data: existingQuotation } = useQuotation(
  ...
  useEffect(() => {
    if (existingQuotation && isEditing) {
      ...
      if (existingQuotation.issueDate)
        setIssueDate(
          new Date(existingQuotation.issueDate).toISOString().split("T")[0],
        );
      if (existingQuotation.expiryDate)
        setExpiryDate(
          new Date(existingQuotation.expiryDate).toISOString().split("T")[0],
        );
