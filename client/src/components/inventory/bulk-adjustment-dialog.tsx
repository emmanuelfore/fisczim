        ...
        <div className="flex flex-col md:flex-row h-full relative">
          <DialogClose asChild>
            <Button
              variant="ghost"
              size="icon"
              className="absolute right-4 top-4 rounded-full text-slate-300 hover:text-slate-600 hover:bg-slate-100 z-50 md:hidden"
            >
              <X className="w-5 h-5" />
            </Button>
          </DialogClose>
          ...
          <div className="w-full md:w-[320px] h-[300px] md:h-full bg-slate-50 border-b md:border-b-0 md:border-r border-slate-200/60 flex flex-col p-6">
            ...
            <div className="flex items-center gap-2 mb-4">
              <Button
                ...
                onClick={() => addProducts(filteredProducts as Product[])}
              >
                Add Visible
              </Button>
              <Button
                ...
                variant="ghost"
                size="sm"
                className="h-8 rounded-xl text-[10px] font-black text-slate-500"
            ...
            <ScrollArea className="flex-1 -mx-2 px-2">
              <div className="space-y-1">
                {filteredProducts.map((p) => (
                  ...
                  >
                    <p className=" font-bold text-slate-700 group-hover:text-primary leading-tight mb-1">
                      {p.name}
                    </p>
                    <div className="flex items-center justify-between">
                      ...
                      <Badge
                        variant="secondary"
                        className="bg-slate-200/50 text-slate-500 text-[9px] font-bold py-0 h-4"
                      >
          ...
          <div className="flex-1 bg-white flex flex-col p-6 md:p-8 overflow-hidden relative">
            <DialogClose asChild>
              <Button
                variant="ghost"
                size="icon"
                className="absolute right-4 top-4 rounded-full text-slate-300 hover:text-slate-600 hover:bg-slate-100 z-50 hidden md:flex"
              >
                <X className="w-5 h-5" />
              </Button>
            </DialogClose>
                  ...
                  <div className="space-y-6 pb-6">
                    {fields.length === 0 ? (
                      <div className="flex flex-col items-center justify-center py-20 text-slate-400 opacity-50">
                        ...
                        <p className="text-xs">
                          Search and click a product on the left to add it
                        </p>
                      </div>
                    ) : (
                      fields.map((field, index) => {
                        ...
                        return (
                          <div
                            key={field.id}
                            className="relative bg-slate-50/50 border border-slate-100 rounded-3xl p-5 group hover:border-slate-200 transition-all"
                          >
                            <Button
                              type="button"
                              variant="ghost"
                                  ...
                                  <FormItem>
                                    ...
                                    <FormControl>
                                      <QuantityInput
                                        ...
                                        {...field}
                                        className="rounded-xl h-10 bg-white border-slate-200 focus:ring-amber-500/10 font-mono font-bold"
                                      />
                                    </FormControl>
                                  </FormItem>
                                )}
                                ...
                                <FormItem className="mt-3">
                                  <FormControl>
                                    <Input
                                      ...
                                      {...field}
                                      className="rounded-xl h-9 bg-white/50 border-slate-100 text-xs focus:ring-amber-500/10 italic"
                                    />
                                  </FormControl>
                                </FormItem>
                              )}
                ...
                <div className="pt-6 mt-6 border-t border-slate-100 space-y-4">
                      ...
                      <FormItem>
                        ...
                        <FormControl>
                          <Textarea
                            ...
                            className="resize-none h-20 rounded-2xl bg-slate-50 border-slate-100 focus:ring-primary/10 "
                            {...field}
                          />
                        </FormControl>
                      </FormItem>
                  ...
                  <div className="flex justify-end gap-3 pt-2">
                    <Button
                      ...
                      className="rounded-2xl px-6 text-slate-500 font-bold"
                    >
                      Cancel
                    </Button>
                    <Button
