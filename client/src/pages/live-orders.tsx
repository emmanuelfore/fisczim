      ...
      <div className="flex flex-col gap-6">
        <div className="flex flex-col md:flex-row gap-4 items-center justify-between bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
          ...
          <div className="flex gap-2 w-full md:w-auto">
            <Badge
              variant="outline"
              className="px-4 py-2 border-slate-200 bg-slate-50 text-slate-600 font-bold"
            >
            ...
            <Table>
              <TableHeader>
                <TableRow className="bg-slate-50/80">
                  <TableHead className="py-4 pl-6 font-black uppercase tracking-wider text-[10px] text-slate-400">
                    Order
                  </TableHead>
                  <TableHead className="py-4 font-black uppercase tracking-wider text-[10px] text-slate-400">
                    Type
                  </TableHead>
                  <TableHead className="py-4 font-black uppercase tracking-wider text-[10px] text-slate-400">
                    Customer
                  ...
                  <TableHead className="py-4 pr-6 text-right font-black uppercase tracking-wider text-[10px] text-slate-400">
                    Actions
                  </TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredOrders?.map((order: any) => (
                  <TableRow
                    ...
                    onClick={() => setLocation(`/invoices/${order.id}`)}
                  >
                    <TableCell className="py-4 pl-6">
                      <div className="flex items-center gap-3">
                        <div className="h-9 w-9 rounded-xl bg-violet-50 flex items-center justify-center text-violet-600 font-black  border border-violet-100">
                          {order.orderNumber}
                        </div>
                        <div className="flex flex-col">
                          ...
                          <span className="text-[10px] text-slate-400 font-medium italic">
                            {format(new Date(order.issueDate), "HH:mm")}
                          </span>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      {order.diningOption === "delivery" ? (
                        <div className="flex flex-col gap-1">
                          <Badge className="bg-amber-100 text-amber-700 border-amber-200 hover:bg-amber-100 w-fit text-[10px] font-black">
                            DELIVERY
                          </Badge>
                      ...
                      <div className="flex items-center justify-end gap-2">
                        {order.orderStatus === "pending" && (
                          <Button
                            size="sm"
                            variant="outline"
                            className="h-8 text-[11px] font-bold border-blue-200 text-blue-600 bg-blue-50/50 hover:bg-blue-50"
                            onClick={() =>
                        ...
                        {order.orderStatus === "preparing" && (
                          <Button
                            size="sm"
                            variant="outline"
                            className="h-8 text-[11px] font-bold border-emerald-200 text-emerald-600 bg-emerald-50/50 hover:bg-emerald-50"
                            onClick={() =>
                        ...
                        {order.orderStatus === "ready" && (
                          <Button
                            size="sm"
                            variant="outline"
                            className="h-8 text-[11px] font-bold bg-slate-900 text-white hover:bg-slate-800"
                            onClick={() =>
