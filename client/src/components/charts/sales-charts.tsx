  ...
  const { data: chartData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await apiFetch(
        `/api/reports/charts/sales-by-category/${companyId}?${params.toString()}`,
      );
      if (!res.ok) return [];
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              <Tooltip
                formatter={(value: any) => [
                  `${consolidatedSymbol}${value.toFixed(2)}`,
                  "Sales",
                ]}
                contentStyle={{
                  backgroundColor: "#fff",
          ...
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data}>
              ...
              <YAxis
                ...
                tickLine={false}
                axisLine={false}
                tickFormatter={(v) => `${consolidatedSymbol}${v}`}
              />
              <Tooltip
                cursor={{ fill: "#f8fafc" }}
                formatter={(value: any) => [
...
export function SalesByUserChart({
  companyId,
  dateRange,
  consolidatedSymbol = "$",
  consolidatedRate = 1,
  ...
  const { data: chartData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await apiFetch(
        `/api/reports/charts/sales-by-user/${companyId}?${params.toString()}`,
      );
      if (!res.ok) return [];
      return await res.json();
    },
