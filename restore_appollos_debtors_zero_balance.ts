...
async function main() {
    console.log(`Starting restore for company ${COMPANY_ID} with ZERO balances...`);
    ...
    for (const row of rows as any[]) {
        ...
        // Force balance to 0 as requested
        const balance = 0; 
        ...
        if (existing.length > 0) {
            ...
            await db.update(customers)
                .set({
                    ...
                    phone: existing[0].phone || c.phone, 
                })
    ...
    console.log(`Restore complete! Inserted: ${inserted}, Updated: ${updated}`);
    process.exit(0);
}
