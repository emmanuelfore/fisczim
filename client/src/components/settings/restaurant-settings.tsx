...
export function RestaurantSettings({ company, onUpdate }: Props) {
  ...
  const { data: sections, isLoading: isLoadingSections } = useQuery<Section[]>({
    ...
    queryFn: async () => {
      const res = await apiFetch(
        `/api/companies/${company.id}/restaurant/sections`,
      );
      return res.json();
    },
  });
  ...
  const { data: tables, isLoading: isLoadingTables } = useQuery<Table[]>({
    ...
    queryFn: async () => {
      ...
      const res = await apiFetch(
        `/api/restaurant/sections/${activeSectionId}/tables`,
      );
      return res.json();
    },
  ...
  const handleToggleEnabled = async (val: boolean) => {
    try {
      ...
      toast({
        title: val ? "Restaurant Mode Enabled" : "Restaurant Mode Disabled",
      });
    } catch (e: any) {
            ...
            <div className="flex items-center gap-5">
              ...
              <div className="space-y-1">
                ...
                <p className=" text-muted-foreground">
                  Enable table management, floor plans, and BOM recipes.
                </p>
              </div>
            </div>
                ...
                <DialogContent className="rounded-[2rem] border-none shadow-2xl p-0 overflow-hidden bg-white max-w-sm">
                  ...
                  <div className="p-8 space-y-6">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (newSectionName)
                          createSectionMutation.mutate(newSectionName);
                      }}
                      disabled={
                        createSectionMutation.isPending || !newSectionName
                      }
                    >
                ...
                sections?.map((section) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <MapIcon
                        className={`w-5 h-5 ${activeSectionId === section.id ? "text-white/80" : "text-slate-300"}`}
                      />
                    ...
                    <div className="flex items-center gap-2">
                      <Badge
                        variant="secondary"
                        className={`border-none rounded-lg px-2 ${activeSectionId === section.id ? "bg-white/20 text-white" : "bg-slate-100 text-slate-400"}`}
                      >
                        ID: {section.id}
                      </Badge>
                    </div>
          ...
          <div className="space-y-6">
            {activeSectionId ? (
                        ...
                        <div className="grid grid-cols-2 gap-6">
                          ...
                          <div className="space-y-2">
                            ...
                            <Select
                              ...
                              onValueChange={(v: any) =>
                                setNewTableData({ ...newTableData, shape: v })
                              }
                            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {isLoadingTables ? (
                    ...
                    tables?.map((table) => (
                      <Card
                        key={table.id}
                        className="rounded-[1.5rem] border-none shadow-sm hover:shadow-xl transition-all duration-300 group overflow-hidden bg-white"
                      >
                        <CardContent className="p-0">
                          ...
                          <div className="p-3 bg-slate-50/50 flex gap-2 border-t border-slate-50">
                            <Button
                              variant="ghost"
                              size="sm"
                              className="flex-1 rounded-xl h-9 font-bold text-xs text-slate-500 hover:text-indigo-600 hover:bg-white shadow-none"
                            >
                            ...
                            <Button
                              variant="ghost"
                              size="sm"
                              className="rounded-xl h-9 w-9 text-slate-300 hover:text-red-500 hover:bg-white shadow-none"
                            >
                  ...
                  {(!tables || tables.length === 0) && (
                    <div className="col-span-full text-center py-20 bg-white/50 rounded-[2.5rem] border-4 border-dashed border-slate-100">
                      ...
                      <p className=" font-black text-slate-400 uppercase tracking-widest">
                        No tables defined in this section
                      </p>
                    </div>
                  )}
                </div>
              ...
              <div className="flex flex-col items-center justify-center p-20 bg-indigo-50/30 rounded-[3rem] border-2 border-dashed border-indigo-100 text-center">
                ...
                <p className=" text-indigo-600/60 max-w-xs mx-auto">
                  Click on a floor section on the left to manage its restaurant
                  tables.
                </p>
              </div>
            )}
        ...
        <div className="p-20 flex flex-col items-center justify-center text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className=" text-slate-400 max-w-sm">
              Enable Restaurant Mode at the top to configure your floor plan,
              sections, and table layouts.
            </p>
          </div>
        </div>
      )}
