...
export default function TaxReportsPage() {
  ...
  const { data: sales, isLoading: isLoadingSales } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await apiFetch(
        `/api/companies/${companyId}/reports/sales?${params.toString()}`,
      );
  ...
  const { data: zimraLogs, isLoading: isLoadingLogs } = useQuery({
    ...
    queryFn: async () => {
      const res = await apiFetch(
        `/api/companies/${companyId}/zimra-logs?limit=10`,
      );
      if (!res.ok) return [];
      return await res.json();
    },
  ...
  // Calculations
  const vatStandard =
    sales?.reduce((acc: number, inv: any) => {
  ...
  const totalSales =
    sales?.reduce((acc: number, inv: any) => acc + Number(inv.total), 0) || 0;

  // Fiscal Status Distribution
  const fiscalStatusData =
    sales?.reduce((acc: any, inv: any) => {
  ...
  const handleExportCsv = () => {
    ...
    link.setAttribute(
      "download",
      `Tax_Report_${format(new Date(), "yyyyMMdd")}.csv`,
    );
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <Card className="border-none shadow-sm bg-blue-50/50">
            <CardContent className="p-6">
              ...
              <h3 className="text-2xl font-black text-blue-900">
                {sales?.filter((i: any) => i.validationStatus === "valid")
                  .length || 0}{" "}
                / {sales?.length || 0}
              </h3>
            </CardContent>
          ...
          <Card className="border-none shadow-sm bg-rose-50/50">
            <CardContent className="p-6">
              <p className="text-xs font-bold text-rose-400 uppercase tracking-widest mb-1">
                Failed/Red Status
              </p>
              <h3 className="text-2xl font-black text-rose-900">
                {sales?.filter(
                  (i: any) =>
                    i.validationStatus === "red" ||
                    i.validationStatus === "invalid",
                ).length || 0}
              </h3>
            </CardContent>
          </Card>
          ...
          <Card className="lg:col-span-1 border-none shadow-sm">
            <CardHeader>
              <CardTitle className="text-lg font-bold">
                Fiscalization Health
              </CardTitle>
              <CardDescription>
                Status distribution of issued receipts
              </CardDescription>
            </CardHeader>
                    ...
                    .map((inv: any) => (
                      <TableRow key={inv.id}>
                        ...
                        <TableCell className="text-xs text-slate-500">
                          {format(
                            new Date(inv.issueDate || inv.createdAt),
                            "dd MMM yyyy",
                          )}
                        </TableCell>
                        <TableCell>
