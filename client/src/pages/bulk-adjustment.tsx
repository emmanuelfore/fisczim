                    ...
                    <PopoverContent className="w-[400px] p-0 rounded-3xl overflow-hidden shadow-2xl border-slate-100" align="end">
                        <Command className="rounded-none">
                            ...
                            <CommandList className="max-h-[300px]">
                                <CommandEmpty className="py-6 text-center text-slate-400 font-bold text-xs">No matching products found.</CommandEmpty>
                                <CommandGroup>
                                    {trackedProducts.map(p => (
                                        ...
                                        >
                                            <div className="flex flex-col">
                                                <span className="font-bold text-slate-700 group-hover:text-primary transition-colors">{p.name}</span>
                                                <span className="text-[10px] font-mono text-slate-400 uppercase">{p.sku || 'No SKU'}</span>
                                            </div>
                                            <Badge variant="secondary" className="bg-slate-100 text-slate-500 text-[9px] font-bold">
                ...
                <CardHeader className="border-b border-slate-50 px-8 py-5 bg-slate-50/30">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <CardTitle className="text-lg font-bold text-slate-900 leading-none mb-1">Adjustment Matrix</CardTitle>
                                <CardDescription className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">Digital Stock Reconciliation Log</CardDescription>
                            </div>
                        </div>
                                ...
                                <div className="p-0">
                                    {fields.length === 0 ? (
                                        <div className="flex flex-col items-center justify-center py-32 text-slate-400 opacity-50">
                                            ...
                                            <p className="font-black text-lg text-slate-900 mb-1">List is Empty</p>
                                            <p className="text-[11px] font-bold uppercase tracking-widest">Add products using the button above</p>
                                        </div>
                                    ) : (
                                        <table className="w-full text-left border-collapse table-fixed">
                                            <thead className="bg-slate-50/50 sticky top-0 z-10 backdrop-blur-md">
                                                <tr className="border-b border-slate-100">
                                                    <th className="px-6 py-3 font-black text-slate-400 uppercase tracking-[0.2em] text-[9px] w-12 text-center pointer-events-none">#</th>
                                                                        ...
                                                                        <FormItem>
                                                                            <Select onValueChange={field.onChange} defaultValue={field.value}>
                                                                                ...
                                                                                <SelectContent className="rounded-2xl shadow-2xl p-1 border-slate-50">
                                                                                    <SelectItem value="ADJUSTMENT" className="rounded-xl text-[11px] font-bold">Adjustment</SelectItem>
                                                                                    <SelectItem value="SHRINKAGE" className="rounded-xl text-[11px] font-bold">Shrinkage</SelectItem>
                                                                                    <SelectItem value="CORRECTION" className="rounded-xl text-[11px] font-bold">Correction</SelectItem>
                                                                                    <SelectItem value="DAMAGE" className="rounded-xl text-[11px] font-bold text-rose-500">Damage</SelectItem>
                                                                                    <SelectItem value="EXPIRY" className="rounded-xl text-[11px] font-bold">Expiry</SelectItem>
                                                                                </SelectContent>
                                                                            </Select>
                                                                        </FormItem>
                                                                    )}
                                ...
                                <div className="p-6 border-t border-slate-100 bg-slate-50/30 flex flex-col gap-4">
                                    <div className="flex items-center justify-between p-6 rounded-[2rem] bg-slate-900 text-white shadow-xl">
                                        <div className="flex gap-8 items-center">
                                            <div className="flex flex-col">
                                                <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Batch</span>
                                                <span className="text-xl font-black">{fields.length} <span className="text-[10px] font-bold text-slate-500">Unprocessed</span></span>
                                            </div>
                                        ...
                                        <div className="flex gap-3">
                                            <Button type="button" variant="ghost" onClick={() => form.reset()} className="rounded-xl px-6 h-11 font-black uppercase text-[10px] tracking-widest text-slate-400 hover:text-white hover:bg-white/5 transition-all">
                                                Reset
                                            </Button>
                                            <Button 
                                                type="submit" 
