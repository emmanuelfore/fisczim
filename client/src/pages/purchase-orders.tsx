...
import { useCreatePurchaseOrder, usePurchaseOrders, useUpdatePurchaseOrderStatus, type PurchaseOrder } from "@/hooks/use-purchase-orders";
...
export default function PurchaseOrdersPage() {
  ...
  const setStatus = (order: PurchaseOrder, status: PurchaseOrder["status"]) => {
    updateStatus({ id: order.id, status }, {
      onSuccess: () => toast({ title: "Purchase order updated", description: `${order.poNumber} is now ${status.toLowerCase()}.` }),
  ...
  return (
    <Layout>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/80 border-b border-slate-100">
                <th className="p-5 font-black text-slate-400 uppercase tracking-widest text-[10px]">PO No</th>
                <th className="p-5 font-black text-slate-400 uppercase tracking-widest text-[10px]">Supplier</th>
                <th className="p-5 font-black text-slate-400 uppercase tracking-widest text-[10px]">Expected</th>
                <th className="p-5 font-black text-slate-400 uppercase tracking-widest text-[10px]">Lines</th>
                <th className="p-5 font-black text-slate-400 uppercase tracking-widest text-[10px]">Total</th>
            ...
            <tbody>
              {isLoading ? (
                <tr>
                  <td colSpan={7} className="p-12 text-center text-slate-500">Loading purchase orders...</td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                  <td colSpan={7} className="p-12 text-center text-slate-500">
                    <p className="font-bold text-lg">No purchase orders found</p>
                ...
                <tr key={order.id} className="border-b border-slate-50 hover:bg-slate-50/60 transition-colors">
                  <td className="p-4">
                    <Badge variant="outline" className="font-mono text-[10px] border-slate-200 bg-white">{order.poNumber}</Badge>
                  </td>
                  <td className="p-4 text-sm font-semibold text-slate-700">{order.supplierName || "N/A"}</td>
                  <td className="p-4 text-sm font-medium text-slate-600">
                    {order.expectedDate ? format(new Date(order.expectedDate), "dd MMM yyyy") : "-"}
                  </td>
                  <td className="p-4 text-sm font-semibold text-slate-700">{order.lineCount}</td>
                  <td className="p-4 text-sm font-black text-slate-900">${Number(order.totalCost || 0).toFixed(2)}</td>
                  <td className="p-4">
                    <StatusBadge status={order.status} />
                  </td>
                  <td className="p-4">
                    <div className="flex justify-end">
                      <Select value={order.status} disabled={updatingStatus} onValueChange={(value) => setStatus(order, value as PurchaseOrder["status"])}>
      ...
      <CreatePurchaseOrderDialog companyId={companyId} open={open} onOpenChange={setOpen} />
    </Layout>
  );
}
...
function CreatePurchaseOrderDialog({ companyId, open, onOpenChange }: { companyId: number; open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  const [lines, setLines] = useState<DraftLine[]>([{ productId: "", quantity: "1", unitCost: "0", notes: "" }]);

  const total = lines.reduce((sum, line) => sum + Number(line.quantity || 0) * Number(line.unitCost || 0), 0);

  const addLine = () => setLines((prev) => [...prev, { productId: "", quantity: "1", unitCost: "0", notes: "" }]);
  ...
  const submit = () => {
    ...
    if (!supplierId || items.length === 0) {
      toast({ title: "Missing details", description: "Select a supplier and at least one product line.", variant: "destructive" });
      return;
    }
    ...
    createOrder({
      ...
      onSuccess: () => {
        toast({ title: "Purchase order created", description: "The PO is ready for supplier processing." });
        reset();
        onOpenChange(false);
      },
      ...
      <DialogContent className="max-w-5xl rounded-2xl">
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <Button type="button" variant="outline" size="sm" onClick={addLine} className="rounded-xl font-bold">
          ...
          <div className="space-y-2 max-h-[320px] overflow-y-auto pr-1">
            {lines.map((line, index) => (
              <div key={index} className="grid grid-cols-12 gap-2 items-end rounded-xl border border-slate-100 bg-slate-50/70 p-3">
                <div className="col-span-12 md:col-span-5 space-y-1">
                  <Label className="text-[10px] uppercase tracking-widest text-slate-400">Product</Label>
                  <Select value={line.productId} onValueChange={(value) => {
                    const product = (products as any[]).find((p) => String(p.id) === value);
                    updateLine(index, { productId: value, unitCost: String(product?.costPrice || line.unitCost || "0") });
                  }}>
                ...
                <div className="col-span-4 md:col-span-2 space-y-1">
                  <Label className="text-[10px] uppercase tracking-widest text-slate-400">Qty</Label>
                  <Input type="number" min="0" value={line.quantity} onChange={(event) => updateLine(index, { quantity: event.target.value })} className="bg-white" />
                </div>
                <div className="col-span-4 md:col-span-2 space-y-1">
                  <Label className="text-[10px] uppercase tracking-widest text-slate-400">Unit Cost</Label>
                  <Input type="number" min="0" value={line.unitCost} onChange={(event) => updateLine(index, { unitCost: event.target.value })} className="bg-white" />
                </div>
                <div className="col-span-3 md:col-span-2 text-sm font-black text-slate-800 pb-2">
                  ${(Number(line.quantity || 0) * Number(line.unitCost || 0)).toFixed(2)}
                </div>
                <div className="col-span-1 flex justify-end pb-1">
                  <Button type="button" variant="ghost" size="icon" disabled={lines.length === 1} onClick={() => removeLine(index)}>
        ...
        <div className="space-y-2">
          <Label>Notes</Label>
          <Textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Supplier instructions or delivery notes..." />
        </div>

        <DialogFooter className="items-center gap-3">
          <div className="mr-auto text-sm font-black text-slate-800">Total: ${total.toFixed(2)}</div>
          <Button type="button" variant="outline" onClick={() => onOpenChange(false)} className="rounded-xl">Cancel</Button>
          <Button type="button" onClick={submit} disabled={isPending} className="rounded-xl font-bold">
