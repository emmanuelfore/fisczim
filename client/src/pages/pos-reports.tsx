...
export default function PosReportsPage() {
  ...
  const { data: topProducts, isLoading: isLoadingProducts } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await apiFetch(
        `/api/companies/${companyId}/reports/product-performance?${params.toString()}`,
      );
      if (!res.ok) return [];
      return await res.json();
    },
  ...
  const { data: shifts, isLoading: isLoadingShifts } = useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await apiFetch(
        `/api/pos/reports/shifts?${params.toString()}`,
      );
      if (!res.ok) return [];
      return await res.json();
    },
  ...
  const totalRevenue =
    filteredSales?.reduce((sum: number, s: any) => sum + Number(s.total), 0) ||
    0;
  const fiscalizedTxns =
    sales?.filter((s: any) => s.syncedWithFdms).length || 0;
  const fiscalizationRate =
    totalTxns > 0 ? (fiscalizedTxns / totalTxns) * 100 : 0;
  ...
  const dailySalesGroups = useMemo(() => {
    ...
    sales.forEach((sale: any) => {
      ...
      groups[dateKey].currencyTotals[cur] =
        (groups[dateKey].currencyTotals[cur] || 0) + Number(sale.total || 0);
    });
  ...
  const handleExportCsv = () => {
    ...
    link.setAttribute(
      "download",
      `POS_Sales_${format(new Date(), "yyyyMMdd")}.csv`,
    );
  ...
  const handleExportDailyCsv = () => {
    ...
    link.setAttribute(
      "download",
      `Daily_Summary_${format(new Date(), "yyyyMMdd")}.csv`,
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  return (
    <Layout>
      <div className="space-y-6">
                ...
                <CardContent className="h-[350px]">
                  {isLoading ? (
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-slate-400 gap-2">
                      ...
                      <p className="">No revenue data for the last 30 days</p>
                    </div>
                  )}
                </CardContent>
              </Card>
              ...
              <div className="p-6 border-b border-slate-100 flex items-center justify-between">
                <div>
                  ...
                  <p className=" text-slate-500 font-medium">
                    Aggregated sales performance by date
                  </p>
                </div>
                <Button
                  size="sm"
                  variant="outline"
                  className="h-9 rounded-xl font-bold flex items-center gap-2"
                      ...
                      >
                        ...
                        <TableCell>
                          <div className="flex flex-wrap gap-2">
                            {Object.entries(group.currencyTotals).map(
                              ([cur, amt]) => (
                                <Badge
                                  key={cur}
                                  variant="outline"
                ...
                <Tabs defaultValue="all-tx" className="w-full">
                  <div className="px-6 pt-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                      ...
                      <CardDescription>
                        Detailed list of all POS sales and fiscal status
                      </CardDescription>
                    </div>
                    <div className="flex items-center gap-3">
                      ...
                      <Link href="/pos/all-sales">
                        <Button
                          size="sm"
                          variant="outline"
                          className="h-9 rounded-xl border-slate-200 font-bold flex items-center gap-2"
                        >
                              ...
                              {isLoading ? (
                                <TableRow>
                                  ...
                                  filteredSales?.filter((s: any) => {
                                    if (tabValue === "fiscalized")
                                      return s.syncedWithFdms;
                                    if (tabValue === "pending")
                                      return !s.syncedWithFdms;
                                    return true;
                                  }) || []
                                    ...
                                    >
                                      ...
                                      <TableCell>
                                        <span className="text-[10px] font-bold text-slate-500">
                                          {sale.receiptCounter
                                            ? `${sale.receiptCounter}/${sale.receiptGlobalNo}`
                                            : "-"}
                                        </span>
                                      </TableCell>
                                      <TableCell>
                                        <div className="flex items-center gap-2">
                                          <div className="w-6 h-6 rounded-full bg-indigo-100 flex items-center justify-center text-[10px] font-bold text-indigo-600">
                                            {sale.cashierName?.[0]?.toUpperCase() ||
                                              "S"}
                                          </div>
                                      ...
                                      <TableCell>
                                        {sale.syncedWithFdms ? (
                                          ...
                                          <Badge
                                            variant="outline"
                                            className="text-amber-600 border-amber-200 bg-amber-50/30 flex items-center gap-1.5 w-fit font-bold rounded-lg py-0.5"
                                          >
          ...
          <TabsContent value="shifts" className="space-y-6">
            ...
            <Card className="border-none shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Cash-only tracking and variance reconciliation
                </CardDescription>
              </CardHeader>
              <CardContent>
                <Table>
                        ...
                        return (
                          <TableRow key={shift.id}>
                            ...
                            <TableCell className="font-mono text-[11px]">
                              ...
                              {shift.endTime && (
                                <div className="text-slate-400">
                                  to {format(new Date(shift.endTime), "HH:mm")}
                                </div>
                              )}
                            </TableCell>
                            <TableCell className="text-right text-xs">
                            ...
                            <TableCell className="text-right text-xs text-red-600">
                              -$
                              {(
                                Number(shift.cashDrops) +
                                Number(shift.cashPayouts)
                              ).toFixed(2)}
                            </TableCell>
                            ...
                            <TableCell className="text-right">
                              {shift.actualCash ? (
                                <span className="text-xs font-black">
                                  ${Number(shift.actualCash).toFixed(2)}
                                </span>
                              ) : shift.status === "closed" ? (
                                <Button
                                  size="sm"
                            ...
                            <TableCell
                              className={cn(
                                ...
                                getVarianceColor(shift.variancePercentage),
                              )}
                            >
                              {shift.cashVariance !== null ? (
                                <span>
                                  {shift.cashVariance > 0 ? "+" : ""}
                                  {shift.cashVariance.toFixed(2)}
                                </span>
                              ) : (
                                "-"
                              )}
                            ...
                            <TableCell>
                              {getStatusBadge(shift.reconciliationStatus)}
                            </TableCell>
                          </TableRow>
                        );
                      })
        ...
        <Dialog
          open={!!reconciliationShift}
          onOpenChange={(open) => !open && setReconciliationShift(null)}
        >
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <DialogTitle>Reconcile Cash Drawer</DialogTitle>
              <DialogDescription>
                Compare actual cash counted against system expectations for{" "}
                <strong>{reconciliationShift?.cashierName}</strong>.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-slate-50 rounded-lg border border-slate-100">
                <div className="flex flex-col">
                  ...
                  <span className="text-2xl font-black text-slate-900">
                    ${Number(reconciliationShift?.expectedCash).toFixed(2)}
                  </span>
                </div>
                <div className="flex flex-col border-l pl-4 border-slate-200">
                  ...
                  {actualCashValue ? (
                    <span
                      className={cn(
                        "text-2xl font-black",
                        Number(actualCashValue) -
                          Number(reconciliationShift?.expectedCash) <
                          0
                          ? "text-red-600"
                          : "text-emerald-600",
                      )}
            ...
            <DialogFooter>
              <Button
                variant="outline"
                onClick={() => setReconciliationShift(null)}
              >
                Cancel
              </Button>
              <Button
