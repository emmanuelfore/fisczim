...
// Password verification — only accepts bcrypt and Supabase PBKDF2 hashes
async function verifyPassword(password: string, hash: string): Promise<boolean> {
  if (!hash || !password) return false;

  // Standard Bcrypt ($2a$, $2b$, $2y$)
  ...
  // Supabase PBKDF2 ($pbkdf2-sha256$)
  if (hash.startsWith('$pbkdf2-sha256$')) {
    return verifySupabasePassword(password, hash);
  }

  // Reject plaintext, MD5, SHA256, and any other non-hashed passwords
  console.warn('[AUTH] Rejecting non-hashed password (plaintext, MD5, or SHA256) — user must reset password');
  return false;
}
