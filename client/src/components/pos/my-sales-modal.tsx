...
export function MySalesModal({
  ...
  const [processingIds, setProcessingIds] = useState<
    Record<number, "printing" | "crediting" | null>
  >({});
  ...
  const handleReprint = async (tx: any) => {
    ...
    try {
      ...
      if (posSettings.silentPrinting) {
        ...
        if (!html)
          throw new Error(
            "Receipt content not found. Please expand the row first.",
          );
  ...
  const handleCreditNote = async (tx: any) => {
    ...
    if (
      !confirm(
        `Are you sure you want to issue a Credit Note for ${tx.invoiceNumber}?`,
      )
    )
      return;
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className={cn(
                      "pl-3 text-left font-bold border-slate-200 bg-white rounded-xl h-10 w-[200px]",
            ...
            <Table>
              <TableHeader className="bg-transparent">
                <TableRow className="hover:bg-transparent border-slate-100">
                  ...
                  <TableHead className="font-black text-[10px] uppercase tracking-widest text-slate-400 text-right pr-6">
                    Actions
                  </TableHead>
                </TableRow>
              </TableHeader>
                    ...
                    <Fragment key={tx.id}>
                      ...
                      >
                        ...
                        <TableCell className=" font-bold text-slate-500">
                          {format(new Date(tx.issueDate), "HH:mm")}
                        </TableCell>
                        ...
                        <TableCell className="text-center">
                          {tx.syncedWithFdms ? (
                            ...
                            <Badge className="bg-amber-50 text-amber-600 border-none rounded-lg px-2 py-0.5 font-black text-[9px] uppercase tracking-widest">
                              Credited
                            </Badge>
                          ) : (
                            <Badge
                      ...
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-400 mt-1">
                            Try another date or process new sales.
                          </p>
                        </div>
                      </div>
                    </TableCell>
