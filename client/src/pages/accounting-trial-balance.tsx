            ...
            <Table>
              <TableHeader className="bg-slate-100/50">
                <TableRow className="hover:bg-transparent border-slate-200/60">
                  <TableHead className="w-[140px] font-black text-slate-500 uppercase text-[10px] tracking-widest pl-8">
                    Account Code
                  </TableHead>
                  <TableHead className="font-black text-slate-500 uppercase text-[10px] tracking-widest">
                    Account Name
                  </TableHead>
                  <TableHead className="text-right font-black text-slate-500 uppercase text-[10px] tracking-widest">
                    Debit
                  </TableHead>
                  <TableHead className="text-right pr-8 font-black text-slate-500 uppercase text-[10px] tracking-widest">
                    Credit
                  </TableHead>
                </TableRow>
              </TableHeader>
                        ...
                        >
                          <TableCell colSpan={4} className="pl-8 py-3">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="font-black text-[11px] uppercase tracking-widest text-slate-900">
                                {type}
                              </span>
                              <Badge
                                variant="secondary"
                                className="bg-white text-[10px] py-0"
                              >
                        ...
                        {!collapsedTypes.includes(type) &&
                          lines.map((line) => (
                            <TableRow
                              ...
                              onClick={() =>
                                setLocation(
                                  `/accounting/ledger/${line.accountId}`,
                                )
                              }
                            >
                              <TableCell className="font-bold text-slate-600 pl-14 font-mono ">
                                {line.accountCode}
                              </TableCell>
          ...
          <div className="p-4 bg-rose-50 border border-rose-200 rounded-2xl flex items-center gap-3 text-rose-800 animate-in fade-in slide-in-from-bottom-2">
            ...
            <div className="flex-1">
              <p className="font-bold">
                The Trial Balance is out of equilibrium by{" "}
                {(totalDebit - totalCredit).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                })}
              </p>
              <p className="text-xs opacity-80">
                This usually indicates a missing ledger entry or a manual
                database intervention. Please audit your recent journal entries.
              </p>
            </div>
          </div>
        )}
